...
const STRAPI_URL =
  firstSet('STRAPI_URL', 'NEXT_PUBLIC_STRAPI_URL', 'VITE_STRAPI_URL') ||
  'https://cms.e-do.studio';
const STRAPI_TOKEN = requireEnv(
  'STRAPI_TOKEN',
  'VITE_STRAPI_TOKEN',
  'STRAPI_API_TOKEN',
);
...
- Key pages: ${keyPages.join(' · ')}`;
    ...
    out.push({
      ...
      title:
        lang === 'fr'
          ? 'E-DO Studio — repères essentiels'
          : 'E-DO Studio — essentials',
      url: SITE_URL,
      body: clip(body),
...
function buildMachineChunks(machinesFr, machinesEn) {
  ...
  for (const lang of ['fr', 'en']) {
    ...
    for (const m of rows ?? []) {
      ...
      const specsLines = (m.specs ?? [])
        .map((s) => `- ${s.label} : ${s.value}`)
        .filter(Boolean);
      const usesLines = (m.usages ?? []).map((u) => u.text).filter(Boolean);
      const ratesLines = (m.pricingRows ?? [])
        .map((r) => {
          if (r.amount == null || r.amount === '')
            return r.label ? `- ${r.label} : sur devis` : null;
          const amt =
        ...
        .filter(Boolean);
      const segment =
        m.slug === 'cyclorama' ? CYCLO_SEGMENT[lang] : `plateau/${m.slug}`;
      ...
      const lines = [
        ...
        m.pricingDescription || other?.pricingDescription
          ? `_${m.pricingDescription || other?.pricingDescription}_`
          : null,
      ];
      ...
      out.push({
        ...
        tags: uniq([
          'plateau',
          m.slug,
          ...(m.usages ?? []).map((u) => u.text || '').slice(0, 6),
        ]),
      });
    }
  }
...
function buildPostProdChunks(typesFr, typesEn) {
  ...
  for (const lang of ['fr', 'en']) {
    ...
    for (const t of rows ?? []) {
      if (!t?.slug) continue;
      const includesLines = (t.includes ?? [])
        .map((i) => i.text)
        .filter(Boolean);
      const priceLines = (t.priceRows ?? [])
        .map((r) => {
          if (r.amount == null || r.amount === '')
            return r.label ? `- ${r.label} : sur devis` : null;
...
function buildBlogChunks(postsFr, postsEn) {
  ...
  for (const lang of ['fr', 'en']) {
    ...
    for (const p of rows ?? []) {
      ...
      const cat =
        p.categories?.[0]?.title || other?.categories?.[0]?.title || '';
...
async function fetchPublicWikiPages() {
  const endpoint = new URL(
    'public-pages',
    WIKI_API_URL.endsWith('/') ? WIKI_API_URL : `${WIKI_API_URL}/`,
  );
  const res = await fetch(endpoint, {
    headers: WIKI_API_TOKEN
  ...
  if (!Array.isArray(data))
    throw new Error('Wiki API: expected an array of pages');
  return data;
}
...
async function buildWikiChunks() {
  ...
  if (pages.length) {
    console.log(
      `Wiki: ${out.length} chunk(s) from ${pages.length} public page(s).`,
    );
  }
  return out;
}
...
async function buildCorpus() {
  ...
  const [machinesBI, postProdBI, blogBI, legalBI, teamBI, siteBI] =
    await Promise.all([
      strapiBilingual('machines', {
        populate: 'specs,usages,pricingRows',
      }).catch((e) => {
        console.warn('machines:', e.message);
        return { fr: { data: [] }, en: { data: [] } };
      }),
      strapiBilingual('post-production-types', {
        populate: 'includes,priceRows',
      }).catch((e) => {
        console.warn('post-production-types:', e.message);
        return { fr: { data: [] }, en: { data: [] } };
      }),
      strapiBilingual('blog-posts', {
        populate: 'categories',
        'pagination[pageSize]': '50',
      }).catch((e) => {
        console.warn('blog-posts:', e.message);
        return { fr: { data: [] }, en: { data: [] } };
      }),
      strapiBilingual('legal-sections', {
        'pagination[pageSize]': '200',
      }).catch((e) => {
        console.warn('legal-sections:', e.message);
        return { fr: { data: [] }, en: { data: [] } };
      }),
      strapiBilingual('team-members', { 'pagination[pageSize]': '50' }).catch(
        (e) => {
          console.warn('team-members:', e.message);
          return { fr: { data: [] }, en: { data: [] } };
        },
      ),
      strapiBilingual('site-setting', {
        populate: 'address,openingHours',
      }).catch((e) => {
        console.warn('site-setting:', e.message);
        return { fr: { data: null }, en: { data: null } };
      }),
    ]);
  ...
  while (true) {
    ...
    if (res.ok) {
      ...
      if (embeddings.length !== items.length) {
        throw new Error(
          `Gemini returned ${embeddings.length} embeddings for ${items.length} requests`,
        );
      }
      return embeddings;
    }
    ...
    if ([429, 500, 502, 503, 504].includes(res.status) && attempt < 5) {
      ...
      const delaySec =
        hinted ?? (res.status === 429 ? 60 : Math.min(30, 2 ** attempt));
      console.warn(
        `Gemini ${res.status} — retrying in ${delaySec}s (attempt ${attempt + 1}/5)`,
      );
...
async function embedChunks(chunks) {
  ...
  console.log(
    `Embedding ${chunks.length} chunks in ${batches.length} batch(es)…`,
  );
...
async function main() {
  ...
  console.log(
    `Embed: ${toEmbedCount} new/changed · Skip: ${corpus.length - toEmbedCount}`,
  );
  ...
  if (DRY_RUN) {
    ...
    for (const c of corpus) {
      console.log(
        `${c.id} [${c.lang}] ${c.kind} · ${Buffer.byteLength(c.body)}b · embed=${c._needsEmbed ? 'yes' : 'no'} · ${c.title.slice(0, 60)}`,
      );
    }
    console.log('--- end ---');
    return;
  ...
  console.log(
    `Upserted ${corpus.length}, pruned ${pruned} stale row(s). (${Date.now() - t0}ms)`,
  );
}
