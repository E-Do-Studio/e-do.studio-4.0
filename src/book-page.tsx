...
const StepIntro = ({ n, lang, t, s, compact }: AnyProps) => (
 ...
 >
 <span className="font-mono text-xs font-normal uppercase tracking-widest text-muted-foreground text-primary shrink-0">
 {n} · {t}
 </span>
 {s && (
...
const CfgChoice = ({ idx, on, onClick, label, desc, sub }: AnyProps) => (
 ...
 <span
 data-cfg-arrow
 className="origin-right text-sm leading-none text-primary opacity-0 -translate-x-1 transition-all duration-200 group-hover:translate-x-0 group-hover:scale-110 group-hover:opacity-100"
 >
 →
 </span>
 )}
...
const Step0Configurator = ({
 ...
 {on && (
 <span className="text-primary text-xs mt-auto">
 ●
 </span>
 )}
 </Button>
...
const Step1Plateau = ({
 ...
 <span
 data-plateau-arrow
 className={`text-primary text-base leading-none transition-all duration-200 origin-right ${on ? '' : 'opacity-0 -translate-x-1 group-hover:opacity-100 group-hover:translate-x-0 group-hover:scale-110'}`}
 >
 →
 </span>
 )}
 ...
 <div
 data-plateau-label
 className={`text-3xl font-light tracking-tight mt-1 transition-transform duration-200 origin-left ${on ? '' : 'group-hover:scale-105'}`}
 >
 {px[lang]}
 </div>
 <div
 ...
 >
 <span
 className={`font-mono text-xs tracking-wide text-muted-foreground uppercase overflow-hidden text-ellipsis`}
 >
 {pr.lbl}
 </span>
...
const Step2Date = ({
 ...
 <div>
 <div className="px-5 md:px-6 border-b border-border flex items-center min-h-11 py-3 md:py-0 md:h-11 box-border gap-3 bg-background flex-wrap sticky top-0 z-10">
 <span className="font-mono text-xs font-normal uppercase tracking-widest text-muted-foreground text-primary whitespace-nowrap">{`06 · ${t('booking.pickADate')}`}</span>
 </div>
 ...
 <span
 aria-hidden
 className="inline-block w-2.5 h-2.5 bg-[oklch(0.86_0.045_82)] border border-[oklch(0.72_0.045_82)]"
 />
 ...
 <span
 aria-hidden
 className="inline-block w-2.5 h-2.5 bg-muted border border-input"
 />
 ...
 <div
 className={`grid grid-cols-7 border-b border-border w-full transition-opacity duration-200 ${availLoading ? 'opacity-60' : ''}`}
 >
 {calCells.map((d, i) => {
 if (d === null)
 return (
 ...
 className={[
 'aspect-[1.6] border-r border-b border-input flex flex-col items-start justify-start text-left font-[inherit] min-w-0 p-1.5 sm:p-2 relative transition-colors duration-100',
 sel
 ...
 : partial
 ? 'bg-[oklch(0.86_0.045_82)] text-foreground cursor-pointer hover:bg-[oklch(0.91_0.03_82)]'
 : tdy
 ...
 <span
 className={`font-mono text-[10px] sm:text-xs tracking-wide uppercase mt-auto ${sel ? 'text-muted-foreground' : partial ? 'text-muted-foreground' : tdy ? 'text-primary/70' : 'text-muted-foreground'}`}
 >
 ...
 <span className="font-mono text-xs font-normal uppercase tracking-widest text-muted-foreground">{t('booking.arrivalTime')}</span>
...
const Step5Team = ({ lang, p, team, setTeam, configSessions }: AnyProps) => {
 ...
 <span
 className={`font-mono text-xs tracking-wider uppercase ${n > 0 ? 'text-primary' : 'text-muted-foreground'}`}
 >
 {n > 0 ? t('booking.included') : t('booking.add')}
 </span>
 <span
 onClick={() => setDays(e.k, n > 0 ? 0 : 1)}
 className={`w-5.5 h-5.5 border-2 ${n > 0 ? 'border-primary bg-primary' : 'border-input bg-background'} inline-flex items-center justify-center text-primary-foreground text-sm font-bold`}
 >
 {n > 0 ? '✓' : ''}
 </span>
 </label>
 ...
 <label className="flex items-center gap-2.5 cursor-pointer select-none">
 <span
 className={`font-mono text-xs tracking-wider uppercase ${onReq ? 'text-primary' : 'text-muted-foreground'}`}
 >
 {t('common.onRequest')}
 </span>
 <span
 onClick={() => toggleReq(e.k)}
 className={`w-5.5 h-5.5 border-2 ${onReq ? 'border-primary bg-primary' : 'border-input bg-background'} inline-flex items-center justify-center text-primary-foreground text-sm font-bold`}
 >
 {onReq ? '✓' : ''}
 </span>
 </label>
...
const Step6Postprod = ({
 ...
 <div className="flex flex-col gap-px bg-border">
 <div
 className={`bg-background px-5 md:px-6 py-5 flex items-center justify-between gap-5 ${enabled ? 'border-l-4 border-l-primary' : 'border-l-4 border-l-transparent'}`}
 >
 <div>
 ...
 <span
 className={`font-mono text-xs tracking-wider uppercase ${enabled ? 'text-primary' : 'text-muted-foreground'}`}
 >
 ...
 <div
 className={`bg-background px-5 md:px-6 py-5 flex items-center justify-between gap-5 ${video ? 'border-l-4 border-l-primary' : 'border-l-4 border-l-transparent'}`}
 >
 <div>
 <div className="text-base font-medium tracking-tight">
 ...
 <div className="flex items-center gap-3">
 <span
 className={`font-mono text-xs tracking-wider uppercase ${video ? 'text-primary' : 'text-muted-foreground'}`}
 >
...
const BentoField = ({ label, children, span, error }: AnyProps) => (
 ...
 >
 <FieldLabel className="font-mono text-xs font-normal uppercase tracking-widest text-muted-foreground text-xs tracking-widest text-muted-foreground">
 {label}
 </FieldLabel>
 {children}
...
const Step7Contact = ({
 ...
 <>
 <div
 className={`bg-background px-3 py-1.5 col-span-1 sm:col-span-2 flex flex-col gap-1 min-h-11 ${errors.typesArticles ? 'ring-1 ring-inset ring-destructive' : ''}`}
 >
 <span className="font-mono text-xs font-normal uppercase tracking-widest text-muted-foreground text-muted-foreground text-xs tracking-widest">
 {t('booking.itemTypes')}
 </span>
 ...
 <label
 className={`col-span-1 sm:col-span-2 bg-background px-3 py-1.5 flex flex-col gap-0.5 cursor-pointer min-h-11 ${errors.cgvAccepted ? 'ring-1 ring-inset ring-destructive' : ''}`}
 >
 <span className="font-mono text-xs font-normal uppercase tracking-widest text-muted-foreground text-muted-foreground text-xs tracking-widest">
 CGV *
 </span>
...
const SidePanel = ({
 ...
 <span className="font-mono text-xs font-normal uppercase tracking-widest text-muted-foreground text-muted-foreground mb-1.5 block">
 {t('booking.dates')}
 </span>
 ...
 <span className="font-mono text-xs font-normal uppercase tracking-widest text-muted-foreground text-muted-foreground mb-2.5 block">
 {t('booking.breakdown')}
 </span>
