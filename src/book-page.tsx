        ...
        <div className="h-18 border-t border-border flex items-center justify-between px-12 shrink-0 bg-white">
          ...
          {step<5 ? (
            ...
            step===5 ? (
              ...
              <button onClick={()=>canNext()&&!saving&&handleSubmit('request')} disabled={!canNext()||saving} className={navBtnOrangeCls + (canNext()&&!saving ? '' : ' opacity-30 cursor-not-allowed')}>
            ...
            <div className="flex gap-2.5">
              ...
              {step===5 ? (
                ...
                <button onClick={()=>canNext()&&!saving&&handleSubmit('booking')} disabled={!canNext()||saving} className={navBtnOrangeCls + (canNext()&&!saving ? '' : ' opacity-30 cursor-not-allowed')}>
  ...
  return (
    <div className="min-w-0 overflow-y-auto h-full">
      ...
      {sessions.length > 1 && (<>
        ...
        <div className="grid bg-white border-t border-b border-foreground" style={{gridTemplateColumns:`repeat(${sessions.length}, minmax(0,1fr))`,gap:1}}>
      ...
      {accQ('projectType', <><div className="px-5 sm:px-6 border-b border-foreground flex items-center min-h-control py-4 sm:py-0 gap-3 box-border"><span className="edo-cell-label text-primary">00 · {lang==='fr'?'Type de projet':'Project type'}</span></div><div className="grid grid-cols-2 gap-px bg-foreground sm:bg-white border-b border-foreground">{PROJECT_TYPES.map((pt,i)=>(<CfgChoice key={pt.k} idx={i+1} on={S.projectType===pt.k} onClick={()=>resetFrom('projectType', pt.k)} label={pt[lang]} desc={pt.desc[lang]}/>))}</div></>)}
      {S.projectType === 'ecom' && (accQ('product', <><div className="px-5 sm:px-6 border-b border-foreground flex items-center min-h-control py-4 sm:py-0 gap-3 box-border"><span className="edo-cell-label text-primary">01 · {lang==='fr'?'Type de produit':'Product type'}</span></div><div className="grid grid-cols-2 sm:grid-cols-3 gap-px bg-foreground sm:bg-white border-b border-foreground">{PRODUCTS.map((p,i)=>(<CfgChoice key={p.k} idx={i+1} on={S.product===p.k} onClick={()=>resetFrom('product', p.k)} label={p[lang]} desc={p.desc[lang]}/>))}</div></>))}
      {S.product === 'pap' && (accQ('method', <><div className="px-5 sm:px-6 border-b border-foreground flex items-center min-h-control py-4 sm:py-0 gap-3 box-border"><span className="edo-cell-label text-primary">02 · {lang==='fr'?'Méthode':'Method'}</span></div><div className="grid grid-cols-2 gap-px bg-foreground sm:bg-white border-b border-foreground">{PAP_METHODS.map((m,i)=>(<CfgChoice key={m.k} idx={i+1} on={S.method===m.k} onClick={()=>resetFrom('method', m.k)} label={m[lang]} desc={m.desc[lang]}/>))}</div></>))}
      {S.product === 'pap' && S.method === 'packshot' && (accQ('submethod', <><div className="px-5 sm:px-6 border-b border-foreground flex items-center min-h-control py-4 sm:py-0 gap-3 box-border"><span className="edo-cell-label text-primary">03 · {lang==='fr'?'Type de packshot':'Packshot type'}</span></div><div className="grid grid-cols-2 sm:grid-cols-3 gap-px bg-foreground sm:bg-white border-b border-foreground">{PAP_PACKSHOT_SUBS.map((sub,i)=>(<CfgChoice key={sub.k} idx={i+1} on={S.submethod===sub.k} onClick={()=>resetFrom('submethod', sub.k)} label={sub[lang]} desc={sub.desc[lang]}/>))}</div></>))}
      {S.product === 'accessoires' && (accQ('submethod', <><div className="px-5 sm:px-6 border-b border-foreground flex items-center min-h-control py-4 sm:py-0 gap-3 box-border"><span className="edo-cell-label text-primary">02 · {lang==='fr'?"Type d'accessoire":'Accessory type'}</span></div><div className="grid grid-cols-2 sm:grid-cols-3 gap-px bg-foreground sm:bg-white border-b border-foreground">{ACCESS_SUBS.map((sub,i)=>(<CfgChoice key={sub.k} idx={i+1} on={S.submethod===sub.k} onClick={()=>resetFrom('submethod', sub.k)} label={sub[lang]} desc={sub.desc[lang]}/>))}</div></>))}
      {((S.product === 'pap' && S.method === 'onmodel') || (S.product === 'accessoires' && S.submethod) || (['eyewear','food','cosmetique','bijoux'].includes(S.product))) && (accQ('media', <><div className="px-5 sm:px-6 border-b border-foreground flex items-center min-h-control py-4 sm:py-0 gap-3 box-border flex-wrap"><span className="edo-cell-label text-primary">{S.product==='pap' ? '03' : S.product==='accessoires' ? '03' : '02'} · {lang==='fr'?'Média':'Media'}</span><span className="font-mono text-label tracking-caption text-muted-foreground ml-3">{lang==='fr'?'(un ou les deux)':'(one or both)'}</span></div><div className="grid grid-cols-2 gap-px bg-foreground sm:bg-white border-b border-foreground">{MEDIA_OPTIONS.map((m,i)=>{ const cur = Array.isArray(S.media) ? S.media : (S.media ? [S.media] : []); const on = cur.includes(m.k); return (<CfgChoice key={m.k} idx={i+1} on={on} onClick={()=>{ const next = on ? cur.filter(x=>x!==m.k) : [...cur, m.k]; setSession({ media: next }); }} label={m[lang]} desc={m.desc[lang]}/>); })}</div></>))}
      {S.product === 'pap' && S.method === 'packshot' && S.submethod && (accQ('quantity', <><div className="px-5 sm:px-6 border-b border-foreground flex items-center min-h-control py-4 sm:py-0 gap-3 box-border"><span className="edo-cell-label text-primary">04 · {lang==='fr'?'Nombre de produits':'Number of products'}</span></div><div className="grid grid-cols-1 gap-px bg-foreground sm:bg-white border-b border-foreground"><div className="bg-white px-4 sm:px-3 py-4 sm:py-2.5 flex flex-col gap-2 min-w-0"><div className="flex items-center gap-1.5 max-w-xs min-w-0"><input value={S.quantity} onChange={e=>setSession({quantity: e.target.value.replace(/\D/g,'')})} placeholder="—" inputMode="numeric" className="flex-1 min-w-0 bg-white border border-border outline-none px-3.5 py-2.5 font-mono text-cell tracking-copy-tight text-foreground text-center"/></div></div></div></>))}
      {S.product === 'pap' && S.method === 'packshot' && S.submethod && (accQ('views', <><div className="px-5 sm:px-6 border-b border-foreground flex items-center min-h-control py-4 sm:py-0 gap-3 box-border flex-wrap"><span className="edo-cell-label text-primary">05 · {lang==='fr'?'Vues par produit':'Views per product'}</span><span className="font-mono text-label tracking-caption text-muted-foreground ml-3">{lang==='fr'?'(multi-sélection)':'(multi-select)'}</span></div><div className="grid gap-px bg-foreground sm:bg-white border-b border-foreground grid-cols-auto-tiles">{PACKSHOT_VIEWS.filter(v => v.k !== '3/4' || S.submethod === 'ghost').map((v,i)=>{ const on = (S.views || []).includes(v.k); return (<button key={v.k} onClick={()=>{ const cur = S.views || []; setSession({ views: cur.includes(v.k) ? cur.filter(x => x !== v.k) : [...cur, v.k] }); }} className={`${on ? 'bg-foreground text-white' : 'bg-white text-foreground'} border-0 px-4 sm:px-3 py-4 sm:py-2.5 text-left cursor-pointer font-inherit flex flex-col gap-1.5 min-h-22 sm:min-h-18 min-w-0`}><span className={`font-mono text-label tracking-meta uppercase ${on ? 'text-white/60' : 'text-muted-foreground'}`}>{String(i+1).padStart(2,'0')}</span><span className="text-detail font-normal tracking-headline">{v[lang]}</span>{on && <span className="text-primary text-caption mt-auto">●</span>}</button>); })}</div></>))}
      {((S.product === 'pap' && S.method === 'onmodel' && (S.media||[]).length > 0) || (S.product === 'accessoires' && S.submethod && (S.media||[]).length > 0) || (['eyewear','food','cosmetique','bijoux'].includes(S.product) && (S.media||[]).length > 0)) && (accQ('qtyViews', <><div className="px-5 sm:px-6 border-b border-foreground flex items-center min-h-control py-4 sm:py-0 gap-3 box-border"><span className="edo-cell-label text-primary">{S.product==='pap' ? '04' : S.product==='accessoires' ? '04' : '03'} · {lang==='fr'?'Produits & vues':'Products & views'}</span></div><div className="grid grid-cols-1 sm:grid-cols-2 gap-px bg-foreground sm:bg-white border-b border-foreground"><div className="bg-white px-4 sm:px-3 py-4 sm:py-2.5 flex flex-col gap-2 min-w-0"><span className="edo-cell-label text-muted-foreground">{lang==='fr'?'Nombre de produits':'Number of products'}</span><div className="flex items-center gap-1.5 min-w-0"><input value={S.quantity} onChange={e=>setSession({quantity: e.target.value.replace(/\D/g,'')})} placeholder="—" inputMode="numeric" className="flex-1 min-w-0 w-full bg-white border border-border outline-none px-3.5 py-2.5 font-mono text-cell tracking-copy-tight text-foreground text-center"/></div></div><div className="bg-white px-4 sm:px-3 py-4 sm:py-2.5 flex flex-col gap-2 min-w-0"><span className="edo-cell-label text-muted-foreground">{lang==='fr'?'Vues par produit':'Views per product'}</span><div className="flex items-center gap-1.5 min-w-0"><input value={S.viewsCount} onChange={e=>setSession({viewsCount: e.target.value.replace(/\D/g,'')})} placeholder="—" inputMode="numeric" className="flex-1 min-w-0 w-full bg-white border border-border outline-none px-3.5 py-2.5 font-mono text-cell tracking-copy-tight text-foreground text-center"/></div></div></div></>))}
      {S.projectType === 'ecom' && S.product && sessionValid(S) && (accQ('postprod', <><div className="px-5 sm:px-6 border-b border-foreground flex items-center min-h-control py-4 sm:py-0 gap-3 box-border"><span className="edo-cell-label text-primary">{lang==='fr'?'Post-production':'Post-production'}</span></div><div className="grid gap-px bg-foreground sm:bg-white border-b border-foreground" style={{gridTemplateColumns:((S.media||[]).includes('video') && S.postprod) ? '1fr 1fr' : '1fr'}}><div className="bg-white px-4 sm:px-3.5 py-4 sm:py-2.5 flex items-center justify-between gap-3"><div><div className="text-detail font-medium tracking-copy-tight">{lang==='fr'?'Post-production par E-DO ?':'Post-production by E-DO?'}</div><div className={`font-mono text-label text-muted-foreground mt-0.5`}>{lang==='fr'?'Prix estimatif affiché — ajusté après brief':'Estimated price shown — adjusted after brief'}</div></div><Toggle on={S.postprod} onClick={()=>setSession({postprod:!S.postprod, postprodVideo: S.postprod ? false : S.postprodVideo})}/></div>{((S.media||[]).includes('video')) && S.postprod && (<div className="bg-white px-4 sm:px-3.5 py-4 sm:py-2.5 flex items-center justify-between gap-3"><div><div className="text-detail font-medium tracking-copy-tight">{lang==='fr'?'Montage vidéo ?':'Video editing?'}</div><div className={`font-mono text-label text-muted-foreground mt-0.5`}>{lang==='fr'?'Uniquement pour les projets vidéo':'Only for video projects'}</div></div><Toggle on={S.postprodVideo} onClick={()=>setSession({postprodVideo:!S.postprodVideo})}/></div>)}</div></>))}
...
const Step1Plateau = ({ lang, plateau, setPlateau, plateaus, togglePlateau, setCycloMode, setSlotType, setHours, onConfigurator }: AnyProps) => (
  <div>
    ...
    <div className="grid grid-cols-3 gap-px bg-foreground border-b border-foreground w-full auto-rows-bento">
...
const Step2Date = ({ lang, p, viewY, viewM, months, days, calCells, selected, setSelected, arrivalHour, setArrivalHour, rentalHours, isPast, nextMonth, prevMonth, refreshKey = 0 }: AnyProps) => {
  ...
  return (<div>
    ...
    <div className="grid grid-cols-5 sm:grid-cols-10 gap-px bg-white border-b border-foreground w-full">{Array.from({length:10},(_,i)=>i+9).map(h=>{
...
const Step3Slot = ({ lang, p, slotType, setSlotType, hours, setHours, cycloMode, setCycloMode }: AnyProps) => {
  ...
  return (<div>
    ...
    {(slotType==='hour' || slotType==='half') && (<div className="grid grid-cols-1 gap-px bg-white border-b border-foreground w-full"><div className="bg-white px-12 py-5 flex items-center justify-between gap-5 flex-wrap"><div className="flex flex-col gap-1"><span className="edo-cell-label">{slotType==='hour' ? (lang==='fr'?"Nombre d'heures":"Number of hours") : (lang==='fr'?"Nombre d'heures · demi-journée":"Hours · half day")}</span><span className="font-mono text-label tracking-caption text-muted-foreground leading-normal whitespace-nowrap">{slotType==='hour' ? (lang==='fr' ? '↗ Dès 4h, bascule en demi-journée. Dès 8h, journée complète.' : '↗ From 4h, switches to half day. From 8h, full day.') : (lang==='fr' ? `↗ 4h = ${p.rates.half} €. Au-delà, prorata (${p.rates.half}/4 × h). Dès 8h, journée complète.` : `↗ 4h = €${p.rates.half}. Beyond, pro-rata (${p.rates.half}/4 × h). From 8h, full day.`)}</span></div><div className="flex items-center gap-3.5"><StepperBtn onClick={()=>{ if (slotType==='hour') setHours(Math.max(1,hours-1)); else { const n=hours-1; if(n<4){setSlotType('hour'); setHours(3);} else setHours(n); } }}>−</StepperBtn><span className="text-page-title font-light tracking-headline min-w-10 text-center">{hours}</span><StepperBtn onClick={()=>{ const n = hours+1; if (n>=8) { setSlotType('full'); setHours(8); } else if (slotType==='hour' && n>=4) { setSlotType('half'); setHours(n); } else { setHours(n); } }}>+</StepperBtn></div></div></div>)}
    {slotType==='full' && (<div className="grid grid-cols-1 gap-px bg-white border-b border-foreground w-full"><div className="bg-white px-12 py-5 flex items-center justify-between gap-5 flex-wrap"><div className="flex flex-col gap-1"><span className="edo-cell-label">{lang==='fr'?'Durée totale':'Total duration'}</span><span className="font-mono text-label tracking-caption text-muted-foreground leading-normal whitespace-nowrap">{(() => { const fullDays = Math.floor(hours/8); const extraH = hours - fullDays*8; if (extraH === 0) { return lang==='fr' ? `↗ ${fullDays} ${fullDays>1?'journées':'journée'} (${hours}h) · ${(p.rates.full*fullDays).toFixed(0)} €` : `↗ ${fullDays} ${fullDays>1?'days':'day'} (${hours}h) · €${(p.rates.full*fullDays).toFixed(0)}`; } const extraAmt = +(p.rates.full / 8 * extraH).toFixed(2); return lang==='fr' ? `↗ ${fullDays} ${fullDays>1?'journées':'journée'} (${fullDays*8}h) + ${extraH}h · ${(p.rates.full*fullDays+extraAmt).toFixed(0)} €` : `↗ ${fullDays} ${fullDays>1?'days':'day'} (${fullDays*8}h) + ${extraH}h · €${(p.rates.full*fullDays+extraAmt).toFixed(0)}`; })()}</span></div><div className="flex items-center gap-3.5"><StepperBtn onClick={()=>{ const n = hours-1; if (n<8) { setSlotType('half'); setHours(7); } else { setHours(n); } }}>−</StepperBtn><span className="text-page-title font-light tracking-headline min-w-16 text-center">{hours}h</span><StepperBtn onClick={()=>setHours(hours+1)}>+</StepperBtn></div></div></div>)}
  </div>);
};
...
const BentoField = ({ label, children, span, error }: AnyProps) => (
  <div className={`bg-white px-3 py-1.5 flex flex-col gap-px min-h-control ${error ? 'ring-1 ring-inset ring-red-400' : ''}`} {...(span ? {style:{gridColumn:span}} : {})}>
...
const Confirmation = ({ lang, openMenu, goto, setLang, plateau, selected, arrivalHour, rentalHours, plateaus, perPlateau, total, rows, contact, months, mode, savedRef }: AnyProps) => {
  ...
  const navBtnPrimaryCls = "edo-focus-ring bg-foreground border-0 cursor-pointer text-white font-mono text-caption tracking-meta uppercase px-cell-lg h-control inline-flex items-center gap-2 transition-all duration-150 hover:scale-102 hover:text-primary";
  const navBtnOrangeCls = "edo-focus-ring bg-primary border-0 cursor-pointer text-white font-mono text-caption tracking-meta uppercase px-cell-lg h-control inline-flex items-center gap-2 transition-all duration-150 hover:scale-102 hover:opacity-90";
  return (<div className="grid w-full gap-px bg-edo-pure-black md:h-full md:grid-cols-app md:grid-rows-app md:overflow-hidden">
    <PageHeader
      lang={lang}
      ...
      onLangToggle={()=>setLang(lang==='fr'?'en':'fr')}
    />
    <div className="bg-edo-pure-black overflow-auto flex flex-col gap-px md:col-span-2 md:row-start-2 md:min-h-0">
      <div className="grid gap-px bg-edo-pure-black grid-cols-1 md:grid-cols-confirmation-hero">
      ...
      <div className="grid grid-cols-4 gap-px bg-edo-pure-black">
      ...
      <div className="grid grid-cols-2 gap-px bg-edo-pure-black">
