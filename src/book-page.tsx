  ...
  return (
    <div className="edo-page-enter grid w-full edo-hairline md:h-full md:overflow-hidden md:grid-cols-book md:grid-rows-app">
      ...
      <form ref={contentScrollRef} name="booking" aria-label="Booking" onSubmit={(e) => e.preventDefault()} className="bg-white overflow-auto flex flex-col md:col-start-2 md:col-span-2 md:row-start-2 md:min-h-0">
        {mode === 'manual' && (
          <div className="flex items-stretch bg-muted min-h-control box-border shrink-0 border-b border-hairline">
            <span className="font-mono text-micro tracking-code uppercase text-muted-foreground self-center pl-5 pr-3 flex-1 min-w-0">
            ...
            <div className="flex items-stretch flex-none md:w-1/2">
              <button type="button" onClick={()=>{ setPlateau(null); setPlateaus([]); setPerPlateau({}); setSlotType('hour'); setHours(1); setCycloMode('halfH'); setPaint(false); setKwh(0); setTeam({}); setPp({}); setSelected(null); goToStep(1, 'manual'); }}
                className="edo-focus-ring bg-transparent border-l border-hairline px-5 cursor-pointer font-mono text-micro tracking-code uppercase text-foreground whitespace-nowrap leading-normal inline-flex items-center justify-center md:flex-1 transition-colors duration-150 hover:bg-white">
                ↻ {common.reset[lang]}
              </button>
        ...
        <div ref={innerScrollRef} className="flex-1 overflow-y-auto">
          ...
          {step===6 && (() => {
            ...
            return (<div><div className="px-6 border-b border-hairline flex items-center h-control box-border gap-4 bg-white flex-wrap sticky top-0 z-10"><span className="edo-cell-label text-primary whitespace-nowrap">{lang==='fr'?'Plateau':'Stage'} {String(safeIdx+1).padStart(2,'0')} / {String(list.length).padStart(2,'0')}</span><span className="text-detail font-normal tracking-copy-tight text-foreground">{px ? px[lang] : k}</span><div className="flex gap-1.5 ml-auto">{list.map((kk, i) => { const has = perPlateau[kk] && perPlateau[kk].date; const active = i === safeIdx; return (<button type="button" key={kk} onClick={()=>setDateIdx(i)} className={`${active ? 'bg-foreground text-white border-foreground' : has ? 'bg-primary text-white border-primary' : 'bg-white text-foreground border-border'} border px-2.5 py-1 cursor-pointer font-mono text-label tracking-ui min-w-7 text-center`}>{String(i+1).padStart(2,'0')}{has?' ✓':''}</button>); })}</div></div><Step2Date lang={lang} p={px} viewY={viewY} viewM={viewM} months={months} days={days} calCells={calCells} selected={stSelected} setSelected={(d: DateSelection)=>setSt({date:d})} arrivalHour={stArrival} setArrivalHour={(h: number)=>setSt({arrivalHour:h})} rentalHours={stRentalHours} isPast={isPast} nextMonth={nextMonth} prevMonth={prevMonth} refreshKey={availRefreshKey}/></div>);
          })()}
        </div>
...
const Step0Configurator = ({ lang, global, setGlobal, sessions, setSessions, activeIdx, setActiveIdx, onApply, onSkip, onReset }: AnyProps) => {
  ...
  return (
    <div className="min-w-0 overflow-y-auto h-full">
      <div className="flex items-stretch bg-muted min-h-control box-border sticky top-0 z-local border-b border-hairline">
        <span className="font-mono text-micro tracking-code uppercase text-muted-foreground self-center pl-5 pr-3 flex-1 min-w-0">{lang==='fr'?'Notre configurateur vous accompagne — ou ':'Our configurator guides you — or '}<span className="text-primary font-semibold">{lang==='fr'?'choisissez manuellement →':'pick manually →'}</span></span>
        <div className="flex items-stretch flex-none md:w-1/2">
          <button type="button" onClick={()=>{ setSessions([makeBlankSession()]); setActiveIdx(0); setOpenQ(null); setTouchedQs(new Set()); if (onReset) onReset(); }} className="edo-focus-ring bg-transparent border-l border-hairline px-5 cursor-pointer font-mono text-micro tracking-code uppercase text-foreground whitespace-nowrap leading-normal inline-flex items-center justify-center md:flex-1 transition-colors duration-150 hover:bg-white">↻ {lang==='fr'?'Réinitialiser':'Reset'}</button>
          <button type="button" onClick={onSkip} className="edo-focus-ring bg-primary border-l border-hairline px-5 cursor-pointer font-mono text-label tracking-code uppercase text-white whitespace-nowrap leading-normal font-semibold inline-flex items-center justify-center md:flex-1 transition-all duration-150 hover:opacity-90">{lang==='fr'?'Choisir manuellement':'Choose manually'} →</button>
        </div>
      </div>
      {sessions.length > 1 && (<>
    ...
    <div>
      ...
      {list.map((k, idx) => {
        ...
        return (
          <div key={k}>
            {list.length > 1 && (
              <div className="px-6 border-b border-hairline flex items-center h-control box-border gap-3 bg-white flex-wrap">
...
const Step1Plateau = ({ lang, plateau, setPlateau, plateaus, togglePlateau, setCycloMode, setSlotType, setHours, onConfigurator }: AnyProps) => (
  <div>
    <div className="px-6 border-b border-hairline flex items-center h-control box-border gap-3 bg-white whitespace-nowrap sticky top-0 z-local">
    ...
    <div className="grid grid-cols-3 edo-hairline w-full auto-rows-bento">
...
const Step2Date = ({ lang, p, viewY, viewM, months, days, calCells, selected, setSelected, arrivalHour, setArrivalHour, rentalHours, isPast, nextMonth, prevMonth, refreshKey = 0 }: AnyProps) => {
  ...
  return (<div>
    <div className="px-6 border-b border-hairline flex items-center h-control box-border gap-3 bg-white flex-wrap sticky top-0 z-local"><span className="edo-cell-label text-primary whitespace-nowrap">{`06 · ${fr ? 'Choisir une date' : 'Pick a date'}`}</span></div>
    ...
    <div className="border-b border-hairline bg-white px-6 py-2.5 flex items-center gap-5 flex-wrap">
...
const Step3Slot = ({ lang, p, slotType, setSlotType, hours, setHours, cycloMode, setCycloMode }: AnyProps) => {
  ...
  return (<div>
    ...
    {(slotType==='hour' || slotType==='half') && (<div className="grid grid-cols-1 gap-hairline bg-white border-b border-hairline w-full"><div className="bg-white px-12 py-5 flex items-center justify-between gap-5 flex-wrap"><div className="flex flex-col gap-1"><span className="edo-cell-label">{slotType==='hour' ? (lang==='fr'?"Nombre d'heures":"Number of hours") : (lang==='fr'?"Nombre d'heures · demi-journée":"Hours · half day")}</span><span className="font-mono text-label tracking-caption text-muted-foreground leading-normal whitespace-nowrap">{slotType==='hour' ? (lang==='fr' ? '↗ Dès 4h, bascule en demi-journée. Dès 8h, journée complète.' : '↗ From 4h, switches to half day. From 8h, full day.') : (lang==='fr' ? `↗ 4h = ${p.rates.half} €. Au-delà, prorata (${p.rates.half}/4 × h). Dès 8h, journée complète.` : `↗ 4h = €${p.rates.half}. Beyond, pro-rata (${p.rates.half}/4 × h). From 8h, full day.`)}</span></div><div className="flex items-center gap-3.5"><StepperBtn onClick={()=>{ if (slotType==='hour') setHours(Math.max(1,hours-1)); else { const n=hours-1; if(n<4){setSlotType('hour'); setHours(3);} else setHours(n); } }}>−</StepperBtn><span className="text-page-title font-light tracking-headline min-w-10 text-center">{hours}</span><StepperBtn onClick={()=>{ const n = hours+1; if (n>=8) { setSlotType('full'); setHours(8); } else if (slotType==='hour' && n>=4) { setSlotType('half'); setHours(n); } else { setHours(n); } }}>+</StepperBtn></div></div></div>)}
    {slotType==='full' && (<div className="grid grid-cols-1 gap-hairline bg-white border-b border-hairline w-full"><div className="bg-white px-12 py-5 flex items-center justify-between gap-5 flex-wrap"><div className="flex flex-col gap-1"><span className="edo-cell-label">{lang==='fr'?'Durée totale':'Total duration'}</span><span className="font-mono text-label tracking-caption text-muted-foreground leading-normal whitespace-nowrap">{(() => { const fullDays = Math.floor(hours/8); const extraH = hours - fullDays*8; if (extraH === 0) { return lang==='fr' ? `↗ ${fullDays} ${fullDays>1?'journées':'journée'} (${hours}h) · ${(p.rates.full*fullDays).toFixed(0)} €` : `↗ ${fullDays} ${fullDays>1?'days':'day'} (${hours}h) · €${(p.rates.full*fullDays).toFixed(0)}`; } const extraAmt = +(p.rates.full / 8 * extraH).toFixed(2); return lang==='fr' ? `↗ ${fullDays} ${fullDays>1?'journées':'journée'} (${fullDays*8}h) + ${extraH}h · ${(p.rates.full*fullDays+extraAmt).toFixed(0)} €` : `↗ ${fullDays} ${fullDays>1?'days':'day'} (${fullDays*8}h) + ${extraH}h · €${(p.rates.full*fullDays+extraAmt).toFixed(0)}`; })()}</span></div><div className="flex items-center gap-3.5"><StepperBtn onClick={()=>{ const n = hours-1; if (n<8) { setSlotType('half'); setHours(7); } else { setHours(n); } }}>−</StepperBtn><span className="text-page-title font-light tracking-headline min-w-16 text-center">{hours}h</span><StepperBtn onClick={()=>setHours(hours+1)}>+</StepperBtn></div></div></div>)}
  </div>);
};
...
const Step6Postprod = ({ lang, postprod, setPostprod, plateauKey }: AnyProps) => {
  ...
  return (<div className="px-12 pb-6"><div className="flex flex-col gap-hairline bg-border"><div className={`bg-white px-6 py-5 flex items-center justify-between gap-5 ${enabled ? 'border-l-3 border-l-primary' : 'border-l-3 border-l-transparent'}`}><div><div className="text-cell font-medium tracking-copy-tight">{lang==='fr'?'Post-production par E-DO':'Post-production by E-DO'}</div><div className="font-mono text-label text-muted-foreground mt-1 leading-normal">{lang==='fr' ? 'Sélection, retouche, livraison — chiffrage sur demande selon le volume.' : 'Selection, retouching, delivery — quoted on request based on volume.'}</div></div><div className="flex items-center gap-3"><span className={`font-mono text-label tracking-code uppercase ${enabled ? 'text-primary' : 'text-muted-foreground'}`}>{lang==='fr'?(enabled?'Oui':'Non'):(enabled?'Yes':'No')}</span><Toggle on={enabled} onClick={()=>setPostprod({...postprod, enabled: !enabled, video: !enabled ? video : false})}/></div></div>{enabled && videoAllowed && (<div className={`bg-white px-6 py-5 flex items-center justify-between gap-5 ${video ? 'border-l-3 border-l-primary' : 'border-l-3 border-l-transparent'}`}><div><div className="text-cell font-medium tracking-copy-tight">{lang==='fr'?'Montage vidéo':'Video editing'}</div><div className="font-mono text-label text-muted-foreground mt-1 leading-normal">{lang==='fr' ? 'Uniquement si votre projet inclut de la vidéo — chiffrage sur demande.' : 'Only if your project includes video — quoted on request.'}</div></div><div className="flex items-center gap-3"><span className={`font-mono text-label tracking-code uppercase ${video ? 'text-primary' : 'text-muted-foreground'}`}>{lang==='fr'?(video?'Oui':'Non'):(video?'Yes':'No')}</span><Toggle on={video} onClick={()=>setPostprod({...postprod, video: !video})}/></div></div>)}</div></div>);
};
...
const BentoField = ({ label, children, span, error }: AnyProps) => (
  <div className={`bg-white px-3 py-1.5 flex flex-col gap-hairline min-h-control ${error ? 'ring-1 ring-inset ring-red-400' : ''}`} {...(span ? {style:{gridColumn:span}} : {})}>
...
const Step7Contact = ({ lang, contact, setContact, p, configMode, errors = {} }: AnyProps) => {
  ...
  return (<div>
    <div className="px-6 border-b border-hairline flex items-center h-control box-border gap-3 bg-white flex-wrap sticky top-0 z-local"><span className="edo-cell-label text-primary whitespace-nowrap">05 · {lang==='fr'?'Vos coordonnées':'Your details'}</span></div>
    <div className="grid grid-cols-2 edo-hairline">
      <div className="col-span-2 grid grid-cols-3 edo-hairline"><BentoField label={lang==='fr'?'Marque':'Brand'}><BentoInput name="brand" value={contact.marque} onChange={v=>setContact({...contact,marque:v})} placeholder="—"/></BentoField><BentoField label={lang==='fr'?'Société *':'Company *'} error={errors.societe}><BentoInput name="company" autoComplete="organization" value={contact.societe} onChange={v=>setContact({...contact,societe:v})} placeholder="—"/></BentoField><BentoField label="SIREN *" error={errors.siren}><BentoInput name="siren" inputMode="numeric" value={contact.siren} onChange={v=>setContact({...contact,siren:v})} placeholder="—"/></BentoField></div>
      <BentoField label={lang==='fr'?'Adresse de facturation *':'Billing address *'} span="1 / 3" error={errors.adresseFacturation}><BentoInput name="address" autoComplete="street-address" value={contact.adresseFacturation} onChange={v=>setContact({...contact,adresseFacturation:v})} placeholder="—"/></BentoField>
      ...
      <div className="bg-white px-3 py-1.5 col-span-2 flex flex-col gap-0.5 min-h-control"><span className="edo-cell-label text-muted-foreground text-micro tracking-meta">{lang==='fr'?'Autres informations':'Other information'}</span><textarea name="message" value={contact.autresInfos||''} onChange={e=>setContact({...contact,autresInfos:e.target.value})} placeholder={lang==='fr'?'Contraintes, inspirations, références… (facultatif)':'Constraints, inspirations, references… (optional)'} className="w-full box-border bg-transparent border-0 outline-none p-0 font-inherit text-caption min-h-7 resize-y text-foreground"/></div>
      <label className={`col-span-2 bg-white px-3 py-1.5 flex flex-col gap-0.5 cursor-pointer min-h-control ${errors.cgvAccepted ? 'ring-1 ring-inset ring-red-400' : ''}`}><span className="edo-cell-label text-muted-foreground text-micro tracking-meta">CGV *</span><div className="flex items-center gap-2"><input type="checkbox" name="cgv_accepted" checked={!!contact.cgvAccepted} onChange={e=>setContact({...contact,cgvAccepted:e.target.checked})} className="w-3.5 h-3.5 accent-primary cursor-pointer shrink-0"/><span className="text-caption leading-snug text-foreground">{lang==='fr' ? <>J'accepte les <a href="#" onClick={e=>e.preventDefault()} className="text-primary underline">conditions générales de vente</a> et les modalités de paiement.</> : <>I accept the <a href="#" onClick={e=>e.preventDefault()} className="text-primary underline">terms and conditions of sale</a> and payment terms.</>}</span></div>{errors.cgvAccepted && <span className="text-red-500 text-micro leading-tight">{errors.cgvAccepted}</span>}</label>
    </div>
  </div>);
};
