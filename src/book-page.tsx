  ...
  return (
    <div className="grid w-full gap-px bg-black overflow-y-auto md:h-full md:overflow-hidden md:grid-cols-book-shell md:grid-rows-app">
      ...
      <div ref={contentScrollRef} className="bg-white overflow-auto flex flex-col md:col-start-2 md:row-start-2 md:min-h-0">
        {mode === 'manual' && (
          <div className="px-5 flex items-center justify-between gap-3 bg-muted min-h-control box-border shrink-0 border-b border-black">
            ...
            <div className="flex items-center gap-2 flex-none">
              <button onClick={()=>{ setPlateau(null); setPlateaus([]); setPerPlateau({}); setSlotType('hour'); setHours(1); setCycloMode('halfH'); setPaint(false); setKwh(0); setTeam({}); setPp({}); setSelected(null); setStep(1); }}
                className="bg-transparent border border-border px-2 py-1 cursor-pointer font-mono text-micro tracking-code uppercase text-foreground whitespace-nowrap leading-normal">
                \u21bb {lang==='fr'?'R\u00e9initialiser':'Reset'}
              </button>
              <button onClick={()=>setStep(0)}
                className="bg-primary border border-primary px-3 py-1.5 cursor-pointer font-mono text-label tracking-code uppercase text-white whitespace-nowrap leading-normal font-semibold transition-all duration-150 hover:bg-foreground hover:text-white hover:border-foreground">
        ...
        <div className="h-18 border-t border-border flex items-center justify-between px-12 shrink-0 bg-white">
          {(() => {
            ...
            const navBtnCls = "bg-white border border-border cursor-pointer font-mono text-caption tracking-meta uppercase text-foreground px-5 h-control inline-flex items-center gap-2 transition-all duration-150 hover:scale-102 hover:border-foreground";
            const navBtnPrimaryCls = "bg-foreground border-0 cursor-pointer text-white font-mono text-caption tracking-meta uppercase px-cell-lg h-control inline-flex items-center gap-2 transition-all duration-150 hover:scale-102";
            return (<>
          <button onClick={handleBack} disabled={isFirst && onFirstDateSub} className={"bg-white border border-border cursor-pointer font-mono text-caption tracking-meta uppercase text-foreground px-5 h-control inline-flex items-center gap-2 transition-all duration-150 hover:scale-102 hover:border-foreground" + (isFirst && onFirstDateSub ? ' opacity-30 cursor-not-allowed' : '')}>
            \u2190 {lang==='fr'?'Retour':'Back'}
          </button>
          {step<5 ? (
...
const CfgChoice = ({ idx, on, onClick, label, desc, sub }: AnyProps) => (
  <button onClick={onClick}
    className={`group ${on ? 'bg-foreground text-white' : 'bg-white text-foreground hover:bg-muted'} border-0 outline-none shadow-none p-3.5 text-left cursor-pointer font-inherit flex flex-col gap-1 transition-all duration-150 min-w-0 min-h-28`}>
    <div className="flex justify-between items-start">
      ...
      {on
        ...
        : <span data-cfg-arrow className={`text-primary text-sm leading-none transition-all duration-200 origin-right ${on ? '' : 'opacity-0 -translate-x-1 group-hover:opacity-100 group-hover:translate-x-0 group-hover:scale-115'}`}>\u2192</span>}
    </div>
...
const CfgToggle = ({ on, onClick }: AnyProps) => (
  <button onClick={onClick} className={`w-11.5 h-6.5 ${on ? 'bg-primary' : 'bg-border'} border-0 rounded-full relative cursor-pointer transition-colors duration-150 shrink-0`}>
  ...
  return (
    <div className="min-w-0 overflow-y-auto h-full">
      <div className="px-5 flex items-center justify-between gap-3 bg-muted min-h-control box-border sticky top-0 z-local border-b border-black">
        ...
        <div className="flex items-center gap-2 flex-none">
          <button onClick={()=>{ setSessions([makeBlankSession()]); setActiveIdx(0); setOpenQ(null); setTouchedQs(new Set()); if (onReset) onReset(); }} className="bg-transparent border border-border px-2 py-1 cursor-pointer font-mono text-micro tracking-code uppercase text-foreground whitespace-nowrap leading-normal">\u21bb {lang==='fr'?'R\u00e9initialiser':'Reset'}</button>
          <button onClick={onSkip} className="bg-primary border border-primary px-3 py-1.5 cursor-pointer font-mono text-label tracking-code uppercase text-white whitespace-nowrap leading-normal font-semibold transition-all duration-150 hover:bg-foreground hover:text-white hover:border-foreground">{lang==='fr'?'Choisir manuellement':'Choose manually'} \u2192</button>
        </div>
      </div>
      {sessions.length > 1 && (<>
        <div className="px-6 pt-3.5 pb-1 flex items-baseline justify-between gap-4 flex-wrap"><span className="edo-cell-label text-primary">{lang==='fr'?'Sessions produit':'Product sessions'} \u2014 {sessions.length}</span><button onClick={addSession} className="bg-white border border-border px-3.5 py-2 cursor-pointer font-mono text-label tracking-meta uppercase text-foreground flex items-center gap-2 h-8">+ {lang==='fr'?'Ajouter une session':'Add a session'}</button></div>
        <div className="grid bg-black border-t border-b border-black" style={{gridTemplateColumns:`repeat(${sessions.length}, minmax(0,1fr))`,gap:1}}>
          {sessions.map((s, i) => { const isActive = i === activeIdx; const valid = sessionValid(s); const p = PRODUCTS.find(x => x.k === s.product); const label = s.projectType === 'cyclorama' ? (lang==='fr'?'Cyclorama':'Cyclorama') : (p ? p[lang] : (lang==='fr'?'\u00c0 d\u00e9finir':'To define')); return (<button key={i} onClick={()=>{setActiveIdx(i); setOpenQ(null); setTouchedQs(new Set());}} className={`${isActive ? 'bg-foreground text-white' : 'bg-white text-foreground'} border-0 px-3.5 py-3 text-left cursor-pointer font-inherit flex flex-col gap-1 min-w-0`}><div className="flex items-center justify-between gap-2"><span className={`font-mono text-label tracking-meta ${isActive ? 'text-primary' : 'text-muted-foreground'}`}>{lang==='fr'?'Session':'Session'} {String(i+1).padStart(2,'0')}</span><span onClick={(e)=>{e.stopPropagation(); removeSession(i);}} className={`text-sm cursor-pointer px-1 leading-none ${isActive ? 'text-white/50' : 'text-muted-foreground'}`} title={lang==='fr'?'Retirer':'Remove'}>\u00d7</span></div><div className="text-sm font-normal tracking-headline">{label}</div><div className={`font-mono text-micro tracking-caption ${isActive ? 'text-white/55' : 'text-muted-foreground'}`}>{valid ? (s.projectType==='cyclorama' ? (lang==='fr'?'sur demande':'on request') : `${s.quantity} ${lang==='fr'?'produits':'products'}`) : (lang==='fr'?'incomplet':'incomplete')}</div></button>); })}
        </div>
      </>)}
      {accQ('projectType', <><div className="px-6 border-b border-black flex items-center h-control gap-3 box-border"><span className="edo-cell-label text-primary">00 \u00b7 {lang==='fr'?'Type de projet':'Project type'}</span></div><div className="grid grid-cols-2 gap-px bg-black border-b border-black">{PROJECT_TYPES.map((pt,i)=>(<CfgChoice key={pt.k} idx={i+1} on={S.projectType===pt.k} onClick={()=>resetFrom('projectType', pt.k)} label={pt[lang]} desc={pt.desc[lang]}/>))}</div></>)}
      {S.projectType === 'ecom' && (accQ('product', <><div className="px-6 border-b border-black flex items-center h-control gap-3 box-border"><span className="edo-cell-label text-primary">01 \u00b7 {lang==='fr'?'Type de produit':'Product type'}</span></div><div className="grid grid-cols-3 gap-px bg-black border-b border-black">{PRODUCTS.map((p,i)=>(<CfgChoice key={p.k} idx={i+1} on={S.product===p.k} onClick={()=>resetFrom('product', p.k)} label={p[lang]} desc={p.desc[lang]}/>))}</div></>))}
      {S.product === 'pap' && (accQ('method', <><div className="px-6 border-b border-black flex items-center h-control gap-3 box-border"><span className="edo-cell-label text-primary">02 \u00b7 {lang==='fr'?'M\u00e9thode':'Method'}</span></div><div className="grid grid-cols-2 gap-px bg-black border-b border-black">{PAP_METHODS.map((m,i)=>(<CfgChoice key={m.k} idx={i+1} on={S.method===m.k} onClick={()=>resetFrom('method', m.k)} label={m[lang]} desc={m.desc[lang]}/>))}</div></>))}
      {S.product === 'pap' && S.method === 'packshot' && (accQ('submethod', <><div className="px-6 border-b border-black flex items-center h-control gap-3 box-border"><span className="edo-cell-label text-primary">03 \u00b7 {lang==='fr'?'Type de packshot':'Packshot type'}</span></div><div className="grid grid-cols-3 gap-px bg-black border-b border-black">{PAP_PACKSHOT_SUBS.map((sub,i)=>(<CfgChoice key={sub.k} idx={i+1} on={S.submethod===sub.k} onClick={()=>resetFrom('submethod', sub.k)} label={sub[lang]} desc={sub.desc[lang]}/>))}</div></>))}
      {S.product === 'accessoires' && (accQ('submethod', <><div className="px-6 border-b border-black flex items-center h-control gap-3 box-border"><span className="edo-cell-label text-primary">02 \u00b7 {lang==='fr'?"Type d'accessoire":'Accessory type'}</span></div><div className="grid grid-cols-3 gap-px bg-black border-b border-black">{ACCESS_SUBS.map((sub,i)=>(<CfgChoice key={sub.k} idx={i+1} on={S.submethod===sub.k} onClick={()=>resetFrom('submethod', sub.k)} label={sub[lang]} desc={sub.desc[lang]}/>))}</div></>))}
      {((S.product === 'pap' && S.method === 'onmodel') || (S.product === 'accessoires' && S.submethod) || (['eyewear','food','cosmetique','bijoux'].includes(S.product))) && (accQ('media', <><div className="px-6 border-b border-black flex items-center h-control gap-3 box-border"><span className="edo-cell-label text-primary">{S.product==='pap' ? '03' : S.product==='accessoires' ? '03' : '02'} \u00b7 {lang==='fr'?'M\u00e9dia':'Media'}</span><span className="font-mono text-label tracking-caption text-muted-foreground ml-3">{lang==='fr'?'(un ou les deux)':'(one or both)'}</span></div><div className="grid grid-cols-2 gap-px bg-black border-b border-black">{MEDIA_OPTIONS.map((m,i)=>{ const cur = Array.isArray(S.media) ? S.media : (S.media ? [S.media] : []); const on = cur.includes(m.k); return (<CfgChoice key={m.k} idx={i+1} on={on} onClick={()=>{ const next = on ? cur.filter(x=>x!==m.k) : [...cur, m.k]; setSession({ media: next }); }} label={m[lang]} desc={m.desc[lang]}/>); })}</div></>))}
      {S.product === 'pap' && S.method === 'packshot' && S.submethod && (accQ('quantity', <><div className="px-6 border-b border-black flex items-center h-control gap-3 box-border"><span className="edo-cell-label text-primary">04 \u00b7 {lang==='fr'?'Nombre de produits':'Number of products'}</span></div><div className="grid grid-cols-1 gap-px bg-black border-b border-black"><div className="bg-white px-3 py-2.5 flex flex-col gap-2 min-w-0"><div className="flex items-center gap-1.5 max-w-xs min-w-0"><input value={S.quantity} onChange={e=>setSession({quantity: e.target.value.replace(/\D/g,'')})} placeholder="\u2014" inputMode="numeric" className="flex-1 min-w-0 bg-white border border-border outline-none px-3.5 py-2.5 font-mono text-lg tracking-copy-tight text-foreground text-center"/></div></div></div></>))}
      {S.product === 'pap' && S.method === 'packshot' && S.submethod && (accQ('views', <><div className="px-6 border-b border-black flex items-center h-control gap-3 box-border"><span className="edo-cell-label text-primary">05 \u00b7 {lang==='fr'?'Vues par produit':'Views per product'}</span><span className="font-mono text-label tracking-caption text-muted-foreground ml-3">{lang==='fr'?'(multi-s\u00e9lection)':'(multi-select)'}</span></div><div className="grid gap-px bg-black border-b border-black grid-cols-auto-tiles">{PACKSHOT_VIEWS.filter(v => v.k !== '3/4' || S.submethod === 'ghost').map((v,i)=>{ const on = (S.views || []).includes(v.k); return (<button key={v.k} onClick={()=>{ const cur = S.views || []; setSession({ views: cur.includes(v.k) ? cur.filter(x => x !== v.k) : [...cur, v.k] }); }} className={`${on ? 'bg-foreground text-white' : 'bg-white text-foreground'} border-0 px-3 py-2.5 text-left cursor-pointer font-inherit flex flex-col gap-1.5 min-h-18 min-w-0`}><span className={`font-mono text-label tracking-meta uppercase ${on ? 'text-white/60' : 'text-muted-foreground'}`}>{String(i+1).padStart(2,'0')}</span><span className="text-sm font-normal tracking-headline">{v[lang]}</span>{on && <span className="text-primary text-xs mt-auto">\u25cf</span>}</button>); })}</div></>))}
      {((S.product === 'pap' && S.method === 'onmodel' && (S.media||[]).length > 0) || (S.product === 'accessoires' && S.submethod && (S.media||[]).length > 0) || (['eyewear','food','cosmetique','bijoux'].includes(S.product) && (S.media||[]).length > 0)) && (accQ('qtyViews', <><div className="px-6 border-b border-black flex items-center h-control gap-3 box-border"><span className="edo-cell-label text-primary">{S.product==='pap' ? '04' : S.product==='accessoires' ? '04' : '03'} \u00b7 {lang==='fr'?'Produits & vues':'Products & views'}</span></div><div className="grid grid-cols-2 gap-px bg-black border-b border-black"><div className="bg-white px-3 py-2.5 flex flex-col gap-2 min-w-0"><span className="edo-cell-label text-muted-foreground">{lang==='fr'?'Nombre de produits':'Number of products'}</span><div className="flex items-center gap-1.5 min-w-0"><input value={S.quantity} onChange={e=>setSession({quantity: e.target.value.replace(/\D/g,'')})} placeholder="\u2014" inputMode="numeric" className="flex-1 min-w-0 w-full bg-white border border-border outline-none px-3.5 py-2.5 font-mono text-lg tracking-copy-tight text-foreground text-center"/></div></div><div className="bg-white px-3 py-2.5 flex flex-col gap-2 min-w-0"><span className="edo-cell-label text-muted-foreground">{lang==='fr'?'Vues par produit':'Views per product'}</span><div className="flex items-center gap-1.5 min-w-0"><input value={S.viewsCount} onChange={e=>setSession({viewsCount: e.target.value.replace(/\D/g,'')})} placeholder="\u2014" inputMode="numeric" className="flex-1 min-w-0 w-full bg-white border border-border outline-none px-3.5 py-2.5 font-mono text-lg tracking-copy-tight text-foreground text-center"/></div></div></div></>))}
      {S.projectType === 'ecom' && S.product && sessionValid(S) && (accQ('postprod', <><div className="px-6 border-b border-black flex items-center h-control gap-3 box-border"><span className="edo-cell-label text-primary">{lang==='fr'?'Post-production':'Post-production'}</span></div><div className="grid gap-px bg-black border-b border-black" style={{gridTemplateColumns:((S.media||[]).includes('video') && S.postprod) ? '1fr 1fr' : '1fr'}}><div className="bg-white px-3.5 py-2.5 flex items-center justify-between gap-3"><div><div className="text-sm font-medium tracking-copy-tight">{lang==='fr'?'Post-production par E-DO ?':'Post-production by E-DO?'}</div><div className={`font-mono text-label text-muted-foreground mt-0.5`}>{lang==='fr'?'Prix estimatif affich\u00e9 \u2014 ajust\u00e9 apr\u00e8s brief':'Estimated price shown \u2014 adjusted after brief'}</div></div><CfgToggle on={S.postprod} onClick={()=>setSession({postprod:!S.postprod, postprodVideo: S.postprod ? false : S.postprodVideo})}/></div>{((S.media||[]).includes('video')) && S.postprod && (<div className="bg-white px-3.5 py-2.5 flex items-center justify-between gap-3"><div><div className="text-sm font-medium tracking-copy-tight">{lang==='fr'?'Montage vid\u00e9o ?':'Video editing?'}</div><div className={`font-mono text-label text-muted-foreground mt-0.5`}>{lang==='fr'?'Uniquement pour les projets vid\u00e9o':'Only for video projects'}</div></div><CfgToggle on={S.postprodVideo} onClick={()=>setSession({postprodVideo:!S.postprodVideo})}/></div>)}</div></>))}
      {S.projectType === 'cyclorama' && (<div className="bg-muted p-5 border-t border-b border-black text-center"><div className="text-lg font-normal tracking-headline mb-2">{lang==='fr'?'Cyclorama / Production libre':'Cyclorama / Free production'}</div><div className="text-detail text-muted-foreground max-w-xl mx-auto leading-normal">{lang==='fr'?"Besoin sur-mesure, nous \u00e9tablissons un devis personnalis\u00e9.":"Custom needs \u2014 we'll prepare a tailored quote based on stage, duration and technical resources."}</div></div>)}
      {sessionValid(active) && activeIdx === sessions.length - 1 && (<div className="px-6 py-1.5 flex justify-center items-center bg-white"><button onClick={addSession} className="bg-white border border-border px-4 py-1.5 cursor-pointer font-mono text-label tracking-meta uppercase text-foreground flex items-center gap-2 h-7">+ {lang==='fr'?'Ajouter une autre session produit':'Add another product session'}</button></div>)}
      <div className="h-0 bg-white"/>
      {allValid && recs && (<div className="m-0 bg-foreground text-white"><div className="px-6 py-2 border-b border-white/10 flex items-center justify-between gap-4"><span className={`font-mono text-label tracking-meta uppercase tracking-label text-primary`}>{lang==='fr'?'R\u00e9cap \u2014 recommandation':'Recap \u2014 recommendation'}</span><span className={`font-mono text-micro tracking-ui text-white/45`}>{lang==='fr'?'estimation, ajustable':'estimate, tweakable'}</span></div>{recs.map((r, i) => { const px = BOOK_PLATEAUX.find(x => x.k === r.plateau) || BOOK_PLATEAUX[0]; const p = PRODUCTS.find(x => x.k === r.session.product); const productLabel = r.session.projectType === 'cyclorama' ? (lang==='fr'?'Cyclorama':'Cyclorama') : (p?.[lang] || ''); const totalHours = r.estimatedHours || r.hours || 0; let dur; if (r.onRequest) { dur = lang==='fr' ? 'sur demande' : 'on request'; } else if (totalHours <= 16) { dur = `${totalHours}h`; } else { const d = Math.floor(totalHours / 8); const h = totalHours - d * 8; const dLbl = lang==='fr' ? (d > 1 ? 'jours' : 'jour') : (d > 1 ? 'days' : 'day'); dur = h > 0 ? `${d} ${dLbl} ${lang==='fr'?'et':'+'} ${h}h (${totalHours}h)` : `${d} ${dLbl} (${totalHours}h)`; } return (<div key={i} className="px-6 py-2 border-b border-white/10 grid grid-cols-auto-fluid gap-5 items-baseline"><span className={`font-mono text-label tracking-meta uppercase tracking-label text-white/50`}>{String(i+1).padStart(2,'0')}</span><div><div className="text-sm font-normal tracking-headline mb-px">{px[lang]} <span className="text-white/50 text-caption">\u00b7 {dur}</span></div><div className={`font-mono text-micro tracking-caption text-white/55`}>{productLabel}{r.session.projectType==='cyclorama' ? '' : ` \u00b7 ${r.session.quantity} ${lang==='fr'?'produits':'products'}`}{r.session.projectType==='cyclorama' ? '' : (() => { const q = Number(r.session.quantity)||0; const vc = Number(r.session.viewsCount)||0; const vLen = (r.session.views||[]).length; const v = vc || vLen || 0; const n = q * v; return n > 0 ? ` \u00b7 ${n} ${lang==='fr'?'images':'images'}` : ''; })()}{r.cadence ? ` \u00b7 ${lang==='fr'?`Estimation : ${r.cadence} produits/jour`:`Estimate: ${r.cadence} products/day`}` : ''}</div></div></div>); })}<div className="px-6 py-2.5 flex items-center justify-end gap-3"><button onClick={onApply} className="bg-primary border-0 text-white py-2.5 px-5 cursor-pointer font-mono text-label tracking-meta uppercase tracking-label inline-flex items-center gap-2.5">{lang==='fr'?'Continuer vers la r\u00e9servation':'Continue to booking'}<IconArrowRight width="12" height="12"/></button></div></div>)}
...
const MultiPlateauStep = ({ lang, plateaus, perPlateau, setPerPlateau, fallback, renderOne, topBanner }: AnyProps) => {
  ...
  if (list.length === 0) { return (<div className="p-12 text-center text-muted-foreground font-mono text-xs tracking-ui uppercase">{lang==='fr'?'Aucun plateau s\u00e9lectionn\u00e9 \u2014 revenez \u00e0 l\u2019\u00e9tape 01.':'No stage selected \u2014 go back to step 01.'}</div>); }
      ...
      {list.map((k, idx) => {
        ...
        return (
          <div key={k} className={idx < list.length - 1 ? 'border-b-8 border-b-muted' : ''}>
            {list.length > 1 && (
              <div className="px-6 border-b border-black flex items-center h-control box-border gap-3 bg-white flex-wrap">
...
const Step1Plateau = ({ lang, plateau, setPlateau, plateaus, togglePlateau, setCycloMode, setSlotType, setHours, onConfigurator }: AnyProps) => (
  <div>
    <div className="px-6 border-b border-black flex items-center h-control box-border gap-3 bg-white whitespace-nowrap sticky top-0 z-local">
    ...
    <div className="grid grid-cols-3 gap-px bg-black border-b border-black w-full auto-rows-bento">
    {BOOK_PLATEAUX.map((px,i)=>{
      ...
      return (
        <button key={px.k} onClick={()=>{ togglePlateau(px.k); }}
          className={`group ${on ? 'bg-foreground text-white' : 'bg-white text-foreground hover:bg-muted'} border-0 px-cell py-4 text-left cursor-pointer font-inherit flex flex-col gap-1.5 transition-all duration-150 min-w-0`}>
          <div className="flex justify-between items-start">
            <span className={`font-mono text-label tracking-meta ${on ? 'text-white/60' : 'text-muted-foreground'}`}>{String(i+1).padStart(2,'0')}</span>
            {on ? <span className="text-primary text-lg leading-none">\u25cf</span> : <span data-plateau-arrow className={`text-primary text-base leading-none transition-all duration-200 origin-right ${on ? '' : 'opacity-0 -translate-x-1 group-hover:opacity-100 group-hover:translate-x-0 group-hover:scale-115'}`}>\u2192</span>}
          </div>
...
const Step2Date = ({ lang, p, viewY, viewM, months, days, calCells, selected, setSelected, arrivalHour, setArrivalHour, rentalHours, isPast, nextMonth, prevMonth }: AnyProps) => {
  ...
  return (<div>
    <div className="px-6 border-b border-black flex items-center h-control box-border gap-3 bg-white flex-wrap sticky top-0 z-local"><span className="edo-cell-label text-primary whitespace-nowrap">06 \u00b7 {lang==='fr'?'Choisir une date':'Pick a date'}</span></div>
    <div className="grid gap-px bg-black border-b border-black w-full grid-cols-fluid-two-auto"><div className="bg-white px-12 py-2 flex items-center gap-5 min-w-0 flex-wrap"><h2 className="m-0 text-lg font-light tracking-headline">{months[viewM]} <span className="text-muted-foreground">{viewY}</span></h2><div className="flex gap-3 font-mono text-micro tracking-ui uppercase text-muted-foreground flex-wrap"><LegendChip bg="#fff" br="#141414" lbl={lang==='fr'?'Libre':'Free'}/><LegendChip bg="#f6e4c4" br="#d9b47d" lbl={lang==='fr'?'Partiel':'Limited'}/><LegendChip bg="var(--edo-gray-100)" br="var(--edo-gray-200)" lbl={lang==='fr'?'Indisponible':'Booked'}/><LegendChip bg="var(--edo-orange)" br="var(--edo-orange)" lbl={lang==='fr'?'S\u00e9lectionn\u00e9':'Selected'}/></div></div><button onClick={prevMonth} className="bg-white border-0 cursor-pointer px-cell font-mono text-detail text-foreground">\u2190</button><button onClick={nextMonth} className="bg-white border-0 cursor-pointer px-cell font-mono text-detail text-foreground">\u2192</button></div>
    <div className="grid grid-cols-7 gap-px bg-black border-b border-black w-full">{days.map((d,i)=>(<div key={i} className="bg-muted py-1.5 text-center font-mono text-micro tracking-meta uppercase text-muted-foreground">{d}</div>))}</div>
    <div className="grid grid-cols-7 gap-px bg-black border-b border-black w-full">{calCells.map((d,i)=>{
    ...
    <div className="grid grid-cols-1 gap-px bg-black border-b border-black w-full"><div className="bg-white px-12 py-2.5 flex items-center gap-5 flex-wrap"><span className="edo-cell-label">{lang==='fr'?'Heure d\u2019arriv\u00e9e':'Arrival time'}</span><span className="font-mono text-label tracking-ui text-muted-foreground">{String(arrivalHour).padStart(2,'0')}:00 \u2192 {String(arrivalHour+rentalHours).padStart(2,'0')}:00 \u00b7 {rentalHours}h</span></div></div>
    <div className="grid grid-cols-10 gap-px bg-black border-b border-black w-full">{Array.from({length:10},(_,i)=>i+9).map(h=>{ const on = arrivalHour===h; const endsTooLate = h + rentalHours > 19; const disabled = endsTooLate; return (<button key={h} disabled={disabled} onClick={()=>!disabled && setArrivalHour(h)} title={disabled ? (lang==='fr'?`Termine \u00e0 ${h+rentalHours}h, apr\u00e8s la fermeture`:`Ends at ${h+rentalHours}h, past closing`) : ''} className={`${on ? 'bg-foreground text-white' : disabled ? 'bg-muted text-muted-foreground' : 'bg-white text-foreground'} border-0 ${disabled ? 'cursor-not-allowed' : 'cursor-pointer'} flex items-center justify-center font-mono text-caption tracking-caption min-w-0 aspect-arrival`}>{String(h).padStart(2,'0')}:00</button>); })}</div>
  </div>);
};

const StepperBtn = ({ onClick, children }: AnyProps) => (
  <button onClick={onClick} className="w-7.5 h-8 flex-none basis-8 border border-border bg-white cursor-pointer text-cell text-foreground font-inherit inline-flex items-center justify-center transition-all duration-150 hover:scale-102 hover:border-foreground">{children}</button>
);

const BentoSlotTile = ({ idx, on, onClick, label, sub, desc, price, hint, lang }: AnyProps) => (
  <button onClick={onClick}
    className={`group ${on ? 'bg-foreground text-white' : 'bg-white text-foreground hover:bg-muted'} border-0 px-cell py-4 text-left cursor-pointer font-inherit flex flex-col gap-1.5 transition-all duration-150 min-w-0 min-h-44`}>
    <div className="flex justify-between items-start">
      <span className={`font-mono text-label tracking-meta ${on ? 'text-white/60' : 'text-muted-foreground'}`}>{String(idx).padStart(2,'0')}</span>
      {on ? <span className="text-primary text-lg leading-none">\u25cf</span> : <span data-slot-arrow className={`text-primary text-base leading-none transition-all duration-200 origin-right ${on ? '' : 'opacity-0 -translate-x-1 group-hover:opacity-100 group-hover:translate-x-0 group-hover:scale-115'}`}>\u2192</span>}
    </div>
...
const Step3Slot = ({ lang, p, slotType, setSlotType, hours, setHours, cycloMode, setCycloMode }: AnyProps) => {
  ...
  return (<div>
    ...
    {(slotType==='hour' || slotType==='half') && (<div className="grid grid-cols-1 gap-px bg-black border-b border-black w-full"><div className="bg-white px-12 py-5 flex items-center justify-between gap-5 flex-wrap"><div className="flex flex-col gap-1"><span className="edo-cell-label">{slotType==='hour' ? (lang==='fr'?"Nombre d'heures":"Number of hours") : (lang==='fr'?'Nombre d\u2019heures \u00b7 demi-journ\u00e9e':'Hours \u00b7 half day')}</span><span className="font-mono text-label tracking-caption text-muted-foreground leading-normal whitespace-nowrap">{slotType==='hour' ? (lang==='fr' ? '\u2197 D\u00e8s 4h, bascule en demi-journ\u00e9e. D\u00e8s 8h, journ\u00e9e compl\u00e8te.' : '\u2197 From 4h, switches to half day. From 8h, full day.') : (lang==='fr' ? `\u2197 4h = ${p.rates.half} €. Au-del\u00e0, prorata (${p.rates.half}/4 \u00d7 h). D\u00e8s 8h, journ\u00e9e compl\u00e8te.` : `\u2197 4h = \u20ac${p.rates.half}. Beyond, pro-rata (${p.rates.half}/4 \u00d7 h). From 8h, full day.`)}</span></div><div className="flex items-center gap-3.5"><StepperBtn onClick={()=>{ if (slotType==='hour') setHours(Math.max(1,hours-1)); else { const n=hours-1; if(n<4){setSlotType('hour'); setHours(3);} else setHours(n); } }}>\u2212</StepperBtn><span className="text-4xl font-light tracking-headline min-w-10 text-center">{hours}</span><StepperBtn onClick={()=>{ const n = hours+1; if (n>=8) { setSlotType('full'); setHours(8); } else if (slotType==='hour' && n>=4) { setSlotType('half'); setHours(n); } else { setHours(n); } }}>+</StepperBtn></div></div></div>)}
    {slotType==='full' && (<div className="grid grid-cols-1 gap-px bg-black border-b border-black w-full"><div className="bg-white px-12 py-5 flex items-center justify-between gap-5 flex-wrap"><div className="flex flex-col gap-1"><span className="edo-cell-label">{lang==='fr'?'Dur\u00e9e totale':'Total duration'}</span><span className="font-mono text-label tracking-caption text-muted-foreground leading-normal whitespace-nowrap">{(() => { const fullDays = Math.floor(hours/8); const extraH = hours - fullDays*8; if (extraH === 0) { return lang==='fr' ? `\u2197 ${fullDays} ${fullDays>1?'journ\u00e9es':'journ\u00e9e'} (${hours}h) \u00b7 ${(p.rates.full*fullDays).toFixed(0)} €` : `\u2197 ${fullDays} ${fullDays>1?'days':'day'} (${hours}h) \u00b7 \u20ac${(p.rates.full*fullDays).toFixed(0)}`; } const extraAmt = +(p.rates.full / 8 * extraH).toFixed(2); return lang==='fr' ? `\u2197 ${fullDays} ${fullDays>1?'journ\u00e9es':'journ\u00e9e'} (${fullDays*8}h) + ${extraH}h \u00b7 ${(p.rates.full*fullDays+extraAmt).toFixed(0)} €` : `\u2197 ${fullDays} ${fullDays>1?'days':'day'} (${fullDays*8}h) + ${extraH}h \u00b7 \u20ac${(p.rates.full*fullDays+extraAmt).toFixed(0)}`; })()}</span></div><div className="flex items-center gap-3.5"><StepperBtn onClick={()=>{ const n = hours-1; if (n<8) { setSlotType('half'); setHours(7); } else { setHours(n); } }}>\u2212</StepperBtn><span className="text-4xl font-light tracking-headline min-w-16 text-center">{hours}h</span><StepperBtn onClick={()=>setHours(hours+1)}>+</StepperBtn></div></div></div>)}
  </div>);
};
...
const Step4Equipment = ({ lang, p, paint, setPaint, kwh, setKwh }: AnyProps) => (
  <div className="px-12 pb-6">
    ...
    {p.isCyclo ? (<div className="flex flex-col gap-3"><div className={`flex items-center justify-between py-cell px-5 bg-white ${paint ? 'border-l-3 border-l-primary' : 'border-l-3 border-l-transparent'}`}><div><div className="text-cell font-medium tracking-copy-tight">{lang==='fr'?'Peinture fra\u00eeche du cyclo':'Fresh cyclo paint'}</div><div className="font-mono text-label text-muted-foreground mt-1">{lang==='fr'?'Repeint avant votre arriv\u00e9e \u00b7 forfait':'Repainted before your arrival \u00b7 flat fee'}</div></div><div className="flex items-center gap-3.5"><span className="text-base font-medium">110 €</span><Toggle on={paint} onClick={()=>setPaint(!paint)}/></div></div><div className="py-cell px-5 bg-white"><div className="flex justify-between items-center"><div><div className="text-cell font-medium tracking-copy-tight">{lang==='fr'?'\u00c9lectricit\u00e9 additionnelle':'Extra electricity'}</div><div className="font-mono text-label text-muted-foreground mt-1">1,40 € / kWh</div></div><span className="text-base font-medium">{fmtEUR(kwh*CYCLO_EXTRAS.kwh)} €</span></div><div className="flex items-center gap-3.5 mt-3.5"><StepperBtn onClick={()=>setKwh(Math.max(0,kwh-10))}>\u2212</StepperBtn><div className="flex-1 h-1.5 bg-border relative"><div className="absolute inset-y-0 left-0 bg-primary" style={{right:`${100-Math.min(100,kwh/2)}%`}}/></div><StepperBtn onClick={()=>setKwh(Math.min(200,kwh+10))}>+</StepperBtn><span className="font-mono text-detail min-w-16 text-right">{kwh} kWh</span></div></div></div>) : (<div className="py-6 px-5 bg-white flex items-center gap-3.5"><span className="text-lg text-primary">\u2713</span><span className="text-detail text-foreground">{lang==='fr'?'Mat\u00e9riel standard inclus : fonds, supports, blocs d\u2019alimentation, Wi-Fi pro.':'Standard kit included: backdrops, stands, power blocks, pro Wi-Fi.'}</span></div>)}
  </div>
);
...
const Step7Contact = ({ lang, contact, setContact, p, configMode }: AnyProps) => {
  ...
  return (<div>
    <div className="px-6 border-b border-black flex items-center h-control box-border gap-3 bg-white flex-wrap sticky top-0 z-local"><span className="edo-cell-label text-primary whitespace-nowrap">05 \u00b7 {lang==='fr'?'Vos coordonn\u00e9es':'Your details'}</span></div>
    <div className="grid grid-cols-2 gap-px bg-black border-b border-black">
  ...
  return (<div className="grid w-full gap-px bg-black overflow-y-auto md:h-full md:grid-cols-app md:grid-rows-app md:overflow-hidden">
    ...
    <div className="bg-black overflow-auto flex flex-col gap-px md:col-span-2 md:row-start-2 md:min-h-0">
      <div className="grid gap-px bg-black grid-cols-1 md:grid-cols-confirmation-hero">
        <div className="bg-white pt-7 px-12 pb-6 flex flex-col gap-2.5 min-h-44"><div className="inline-flex items-center gap-2.5 py-1.5 px-3 bg-primary text-white font-mono text-micro tracking-label uppercase self-start">\u25cf {copy.status}</div><h1 className="m-0 text-5xl font-light tracking-display leading-solid text-balance">{copy.title}</h1><p className="m-0 text-detail text-muted-foreground leading-normal max-w-xl text-pretty">{copy.body}</p></div>
        <div className="bg-white p-6 flex flex-col justify-between gap-3.5 min-h-44"><div className="flex flex-col gap-3.5"><div><div className="edo-cell-label text-muted-foreground mb-1">{lang==='fr'?'R\u00e9f\u00e9rence':'Reference'}</div><div className="font-mono text-cell tracking-ui text-foreground">{ref}</div></div><div><div className="edo-cell-label text-muted-foreground mb-1">{lang==='fr'?'\u00c9mis le':'Issued'}</div><div className="font-mono text-xs text-foreground">{new Date().toLocaleDateString(lang==='fr'?'fr-FR':'en-GB',{day:'2-digit',month:'short',year:'numeric',hour:'2-digit',minute:'2-digit'})}</div></div></div><div><div className="edo-cell-label text-muted-foreground mb-1">{lang==='fr'?'Contact':'Contact'}</div><div className="text-detail font-medium tracking-copy-tight">{[contact.prenom,contact.nom].filter(Boolean).join(' ')||'\u2014'}</div><div className="text-caption text-muted-foreground">{contact.email||'\u2014'}</div></div></div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-px bg-black">
        ...
        <div className="bg-white px-5 py-3 flex items-center justify-end"><button onClick={()=>window.location.reload()} className={navBtnPrimaryCls.replace('bg-foreground','bg-primary')}>{lang==='fr'?'Nouvelle demande':'New request'} <IconArrowRight width="14" height="14" stroke="#fff"/></button></div>
      </div>
    </div>
  </div>);
};
