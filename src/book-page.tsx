...
const LegendChip = ({ bg, br, lbl, dot }: AnyProps) => (<span className="inline-flex items-center gap-1.5">{dot ? <span className="w-2.5 h-2.5 rounded-full" style={{background:dot}}/> : <span className="w-2.5 h-2.5" style={{background:bg,border:`1px solid ${br}`}}/>} {lbl}</span>);
  ...
  return (<div>
    ...
    <div className="grid grid-cols-7 gap-px bg-black border-b border-foreground w-full">{calCells.map((d,i)=>{
      ...
      return (<button key={i} disabled={!clickable} onClick={()=>setSelected({y:viewY,m:viewM,d})} title={weekendBlocked ? (lang==='fr'?'Week-end : réservation journée complète uniquement':'Weekend: full-day booking only') : tdy ? (lang==='fr'?'Aujourd\'hui':'Today') : ''}
        className={`${bgClass} border-0 ${clickable ? 'cursor-pointer' : 'cursor-not-allowed'} flex flex-col justify-between text-left font-inherit min-w-0 px-2.5 py-1.5 aspect-calendar`}>
        <span className="flex items-center gap-1"><span className={`text-cell ${sel ? 'font-medium' : tdy ? 'font-semibold' : 'font-normal'} tracking-copy-tight ${past && !sel ? 'line-through decoration-1' : ''}`}>{d}</span>{tdy && !sel && <span className="w-1.5 h-1.5 rounded-full bg-primary flex-none"/>}</span>
