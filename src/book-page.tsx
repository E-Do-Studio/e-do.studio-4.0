  ...
  <button onClick={onClick}
    ...
    <div className="flex justify-between items-start">
      ...
      {on
        ...
        : <span data-cfg-arrow className={`text-primary text-sm leading-none transition-all duration-200 origin-right ${on ? '' : 'opacity-0 -translate-x-1 group-hover:opacity-100 group-hover:translate-x-0 group-hover:scale-110'}`}>→</span>}
    </div>
...
const Step0Configurator = ({ lang, global, setGlobal, sessions, setSessions, activeIdx, setActiveIdx, onApply, onSkip, onReset }: AnyProps) => {
  ...
  return (
    <div className="min-w-0 overflow-y-auto h-full">
      ...
      {S.projectType === 'cyclorama' && (<div className="bg-muted p-5 border-t border-b border-foreground text-center"><div className="text-lg font-normal tracking-headline mb-2">{lang==='fr'?'Cyclorama / Production libre':'Cyclorama / Free production'}</div><div className="text-detail text-muted-foreground max-w-xl mx-auto leading-normal">{lang==='fr'?"Besoin sur-mesure, nous établissons un devis personnalisé.":"Custom needs — we'll prepare a tailored quote based on stage, duration and technical resources."}</div></div>)}
