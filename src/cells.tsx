...
const MarqueeCell = ({ items, pxPerSecond = 50, size = 18 }: MarqueeCellProps) => {
  ...
  <div className="relative flex h-full items-center overflow-hidden bg-white">
    <div ref={trackRef} className="inline-flex whitespace-nowrap pl-5" style={{ animation: `mq ${duration}s linear infinite` }}>
      {[...list, ...list].map((x, i) => (
        <span key={i} className="font-sans text-foreground pr-10" style={{ fontSize: size, fontWeight: 700, letterSpacing: '-0.01em' }}>{x}</span>
      ))}
    </div>
