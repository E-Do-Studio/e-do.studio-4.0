...
const hoverGray = {
  onMouseEnter:e=>{
    ...
    [...e.currentTarget.children].forEach(c=>{
      if (c.tagName === 'STYLE' || c.tagName === 'IMG') return;
      // If this subtree contains the arrow, scale only the text leaves around it.
      if (arr && c.contains(arr)) {
        [...c.children].forEach(gc=>{
          if (gc === arr || gc.contains?.(arr)) return;
          if (gc.tagName === 'STYLE' || gc.tagName === 'IMG') return;
          initHoverEl(gc);
          gc.style.transform = 'scale(1.02)';
        });
        return;
      }
      initHoverEl(c);
    ...
    if (arr) {
      if (!arr.dataset.arrowInited) {
        arr.style.transition = 'transform .25s ease';
        arr.dataset.arrowInited = '1';
      }
      arr.style.transform = 'translateX(4px) scale(1.15)';
    }
  },
  onMouseLeave:e=>{
    ...
    if (arr) arr.style.transform = 'translateX(0) scale(1)';
  },
};
...
const MachineRow = ({ idx, m, lang, onClick, isLast }) => (
  <button onClick={onClick} style={{
    ...
    <IconArrowRight data-arrow width="16" height="16" stroke="#888"/>
  </button>
);
  ...
  return (
      <div style={{height:'100%',overflowY:'auto',overflowX:'hidden',background:'#000',display:'flex',flexDirection:'column',gap:1}}>
        ...
        <div style={{background:'#fff',display:'flex',flexDirection:'column',overflow:'hidden'}}>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'16px 16px 0',gap:12,flexShrink:0}}>
            <div style={{fontSize:18,fontWeight:300,letterSpacing:'-0.03em',lineHeight:1,color:'#141414'}}>
              {lang==='fr'?'Shooting e-commerce':'E-commerce shooting'}
            </div>
            <button onClick={()=>setEcomMode(ecomMode==='type'?'machine':'type')}
              style={{position:'relative',display:'grid',gridTemplateColumns:'1fr 1fr',border:'1px solid #141414',padding:3,gap:3,flexShrink:0,background:'#fff',cursor:'pointer',fontFamily:'inherit'}}>
              <span aria-hidden="true" style={{position:'absolute',top:3,bottom:3,left:ecomMode==='type'?3:'calc(50% + 1.5px)',width:'calc(50% - 4.5px)',background:ecomMode==='type'?'#141414':'var(--edo-orange)',transition:'left .35s cubic-bezier(.6,.05,.2,1), background .35s ease',pointerEvents:'none'}}/>
              {[{v:'type',fr:'Type',en:'Type'},{v:'machine',fr:'Machine',en:'Machine'}].map(o=>{
                const active = ecomMode===o.v;
                return <span key={o.v} style={{position:'relative',zIndex:1,padding:'5px 10px',color:active?'#fff':'#141414',fontFamily:'var(--font-mono)',fontSize:9,letterSpacing:'0.14em',textTransform:'uppercase',transition:'color .25s ease',whiteSpace:'nowrap',pointerEvents:'none',textAlign:'center'}}>{lang==='fr'?o.fr:o.en}</span>;
              })}
            </button>
          </div>
          <div style={{padding:'6px 16px 12px',flexShrink:0}}>
            <div style={{fontSize:10,color:'#888',fontFamily:'var(--font-mono)',letterSpacing:'0.08em',textTransform:'uppercase'}}>
          ...
          {ecomMode==='type' ? (
            <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:1,background:'#000'}}>
              {categories.map((c,i)=>(
                <button key={c.k} onClick={()=>goto('gallery')} style={{background:'#fff',border:0,cursor:'pointer',textAlign:'left',padding:'12px 10px',display:'flex',flexDirection:'column',justifyContent:'space-between',transition:'background .15s',color:'#141414',fontFamily:'inherit',minWidth:0,overflow:'hidden',gap:8,aspectRatio:'1/1'}}
                  onMouseEnter={e=>e.currentTarget.style.background='var(--edo-gray-100)'}
                  onMouseLeave={e=>e.currentTarget.style.background='#fff'}>
                  <CatIcon kind={c.k} size={18}/>
                  <div style={{fontSize:10,fontWeight:500,letterSpacing:'-0.005em',lineHeight:1.2,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{lang==='fr'?c.fr:c.en}</div>
                </button>
              ))}
            </div>
          ) : (
            <div style={{display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:1,background:'#000'}}>
              {ecomMachines.map((m,i)=>(
                <button key={m.slug} onClick={()=>goto('plateau-'+m.slug)} style={{background:'#fff',border:0,cursor:'pointer',textAlign:'left',padding:'14px 14px',display:'flex',flexDirection:'column',justifyContent:'space-between',transition:'background .15s',color:'#141414',fontFamily:'inherit',minWidth:0,overflow:'hidden',gap:10,aspectRatio:'1/1'}}
                  onMouseEnter={e=>e.currentTarget.style.background='var(--edo-gray-100)'}
                  onMouseLeave={e=>e.currentTarget.style.background='#fff'}>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                    <span style={{fontFamily:'var(--font-mono)',fontSize:9,color:'#888',letterSpacing:'0.15em'}}>{String(i+1).padStart(2,'0')}</span>
        ...
        {/* Gallery hero */}
        <button onClick={()=>goto('gallery')} style={{background:'#1a1a1a',position:'relative',overflow:'hidden',aspectRatio:'16/9',width:'100%',border:0,cursor:'pointer',padding:0,flexShrink:0,display:'block'}}
          onMouseEnter={e=>e.currentTarget.style.filter='brightness(0.85)'}
          onMouseLeave={e=>e.currentTarget.style.filter='none'}>
          <img src="./assets/gallery-hero.jpg" alt="" style={{position:'absolute',inset:0,width:'100%',height:'100%',objectFit:'cover',objectPosition:'center'}}/>
          <div style={{position:'absolute',inset:0,background:'linear-gradient(180deg,rgba(0,0,0,.2) 0%,rgba(0,0,0,0) 40%,rgba(0,0,0,.5) 100%)'}}/>
          <div style={{position:'absolute',bottom:16,left:16,right:16,display:'flex',alignItems:'flex-end',justifyContent:'space-between'}}>
            <span style={{fontSize:28,fontWeight:300,letterSpacing:'-0.035em',lineHeight:0.92,color:'#fff'}}>{lang==='fr'?'Galerie':'Gallery'}</span>
            <IconArrowRight width="16" height="16" stroke="#fff"/>
          </div>
        </button>

        {/* Book CTA */}
        <button onClick={()=>goto('book')} style={{background:'var(--edo-orange)',border:0,cursor:'pointer',padding:'20px 16px',display:'flex',alignItems:'center',justifyContent:'space-between',gap:14,fontFamily:'inherit',textAlign:'left',flexShrink:0}}
          ...
          <div style={{display:'flex',flexDirection:'column',gap:4}}>
            <span style={{fontFamily:'var(--font-mono)',fontSize:9,letterSpacing:'0.2em',textTransform:'uppercase',color:'rgba(255,255,255,.75)'}}>{lang==='fr'?'Demander un devis ou':'Request a quote or'}</span>
        ...
        {/* Cyclorama */}
        <button onClick={()=>goto('cyclorama')} style={{background:'#fff',border:0,cursor:'pointer',padding:'16px',display:'flex',alignItems:'center',justifyContent:'space-between',gap:14,fontFamily:'inherit',textAlign:'left',flexShrink:0}}
          onMouseEnter={e=>e.currentTarget.style.background='var(--edo-gray-100)'}
          onMouseLeave={e=>e.currentTarget.style.background='#fff'}>
          <div style={{minWidth:0}}>
            <CellLabel>{lang==='fr'?'Espace':'Space'}</CellLabel>
            <div style={{fontSize:18,fontWeight:300,letterSpacing:'-0.03em',lineHeight:1.1,color:'#141414',marginTop:4}}>Cyclorama</div>
            <div style={{fontSize:10,color:'#888',marginTop:4,fontFamily:'var(--font-mono)',letterSpacing:'0.08em',textTransform:'uppercase'}}>{lang==='fr'?'Production libre · Photo & Vidéo':'Free production · Photo & Video'}</div>
          </div>
          <IconArrowRight width="16" height="16" stroke="#141414"/>
        </button>

        {/* Post-production */}
        <button onClick={()=>goto('postprod')} style={{background:'#fff',border:0,cursor:'pointer',padding:'16px',display:'flex',alignItems:'center',justifyContent:'space-between',gap:14,fontFamily:'inherit',textAlign:'left',flexShrink:0}}
          onMouseEnter={e=>e.currentTarget.style.background='var(--edo-gray-100)'}
          onMouseLeave={e=>e.currentTarget.style.background='#fff'}>
          <div style={{minWidth:0}}>
            <CellLabel>{lang==='fr'?'Service':'Service'}</CellLabel>
            <div style={{fontSize:18,fontWeight:300,letterSpacing:'-0.03em',lineHeight:1.1,color:'#141414',marginTop:4}}>Post-production</div>
            <div style={{fontSize:10,color:'#888',marginTop:4,fontFamily:'var(--font-mono)',letterSpacing:'0.08em',textTransform:'uppercase'}}>{lang==='fr'?'Retouche · Photo & Vidéo':'Retouching · Photo & Video'}</div>
          </div>
          <IconArrowRight width="16" height="16" stroke="#141414"/>
        </button>

        {/* Discovery */}
        <button onClick={()=>goto('discovery')} style={{background:'#141414',border:0,cursor:'pointer',padding:'16px',display:'flex',alignItems:'center',justifyContent:'space-between',gap:14,fontFamily:'inherit',textAlign:'left',flexShrink:0}}
          onMouseEnter={e=>e.currentTarget.style.background='#1e1e1e'}
          onMouseLeave={e=>e.currentTarget.style.background='#141414'}>
          <div style={{minWidth:0}}>
            <CellLabel style={{color:'rgba(255,255,255,.7)'}}>Discovery</CellLabel>
            <div style={{fontSize:18,fontWeight:400,letterSpacing:'-0.015em',lineHeight:1.05,color:'#fff',marginTop:4}}>{lang==='fr'?"Dites-m'en plus":'Tell me more'}</div>
          </div>
          <IconArrowRight width="16" height="16" stroke="#fff"/>
        </button>

        {/* Contact strip */}
        <div style={{background:'#fff',padding:'12px 16px',display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0}}>
          <span style={{fontFamily:'var(--font-mono)',fontSize:11,letterSpacing:'0.05em',color:'#141414'}}>01 44 04 11 49</span>
          <button onClick={()=>goto('contact')} style={{background:'#141414',color:'#fff',border:0,cursor:'pointer',padding:'8px 16px',fontFamily:'var(--font-mono)',fontSize:10,letterSpacing:'0.15em',textTransform:'uppercase',display:'flex',alignItems:'center',gap:8}}>
            {lang==='fr'?'Nous contacter':'Contact us'} <IconArrowRight width="12" height="12"/>
          </button>
        </div>
    ...
    <div style={{
      ...
      <div style={{gridColumn:'1 / 6', gridRow:'1', display:'flex', gap:1, background:'#000', minWidth:0}}>
        ...
        <div style={{flex:'1 1 auto',background:'#fff',display:'flex',alignItems:'center',justifyContent:'center',padding:'0 12px',minWidth:0}}>
          <span style={{fontFamily:'var(--font-mono)',fontSize:12,letterSpacing:'0.12em',textTransform:'uppercase',color:'var(--edo-orange)',whiteSpace:'nowrap'}}>{lang==='fr'?'Lun–Sam · 10—18':'Mon–Sat · 10—18'}</span>
        </div>
      </div>
      ...
      <button onClick={()=>goto('discovery')}
        onMouseEnter={e=>{
          const t = e.currentTarget.querySelector('[data-discovery-title]');
          if (t) t.style.color='var(--edo-orange)';
          const b = e.currentTarget.querySelector('[data-discovery-block]');
          if (b) b.style.transform='scale(1.02)';
          const a = e.currentTarget.querySelector('[data-discovery-arrow]');
        ...
        <div data-discovery-block style={{minWidth:0,display:'flex',flexDirection:'column',gap:4,position:'relative',transition:'transform .25s ease',transformOrigin:'left center'}}>
          <CellLabel style={{color:'rgba(255,255,255,.7)'}}>Discovery</CellLabel>
          <div data-discovery-title style={{fontSize:20,fontWeight:400,letterSpacing:'-0.015em',lineHeight:1.05,color:'#fff',transition:'color .2s ease'}}>
            {lang==='fr'?"Dites-m'en plus":'Tell me more'}
          </div>
        </div>
        <div style={{position:'relative',flexShrink:0}}>
          <IconArrowRight data-discovery-arrow width="16" height="16" stroke="#fff" style={{transition:'transform .25s ease'}}/>
        </div>
      </button>
      ...
      <button onClick={()=>goto('book')} style={{
        ...
        onMouseEnter={e=>{
          e.currentTarget.style.background='#c64f1f';
          const b = e.currentTarget.querySelector('[data-book-block]');
          if (b) b.style.transform='scale(1.02)';
          const a = e.currentTarget.querySelector('[data-book-arrow]');
          if (a) a.style.transform='translateX(4px) scale(1.15)';
        }}
        onMouseLeave={e=>{
        ...
        <div data-book-block style={{display:'flex',flexDirection:'column',gap:4,minWidth:0,transition:'transform .25s ease',transformOrigin:'left center'}}>
          ...
          <span data-book-title style={{fontSize:20,fontWeight:400,letterSpacing:'-0.015em',lineHeight:1.05,color:'#fff'}}>{lang==='fr'?'Réserver':'Book'}</span>
        </div>
        <IconArrowRight data-book-arrow width="16" height="16" stroke="#fff" style={{transition:'transform .25s ease'}}/>
      </button>
      ...
      <div style={{gridColumn:'1 / 4', gridRow:'4', background:'#000', display:'flex', minHeight:0, overflow:'hidden'}}>
        <button onClick={()=>goto('gallery')}
          onMouseEnter={e=>{
            const c = e.currentTarget.querySelector('[data-showreel-play]');
            if (c) c.style.transform='scale(1.15)';
          }}
          onMouseLeave={e=>{
            const c = e.currentTarget.querySelector('[data-showreel-play]');
          ...
          <img src="./assets/showreel-preview.png" alt="" style={{position:'absolute',inset:0,width:'100%',height:'100%',objectFit:'cover',objectPosition:'center'}}/>
          ...
          <div style={{position:'absolute',inset:0,display:'flex',alignItems:'center',justifyContent:'center'}}>
            <div data-showreel-play style={{width:56,height:56,borderRadius:'50%',border:'1px solid rgba(255,255,255,.5)',display:'flex',alignItems:'center',justifyContent:'center',background:'rgba(0,0,0,.25)',backdropFilter:'blur(6px)',transition:'transform .25s ease'}}>
      ...
      <button onClick={()=>goto('cyclorama')} style={{
        ...
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-end',gap:14}}>
          ...
          <div style={{flexShrink:0,display:'flex',alignItems:'center',justifyContent:'center'}}>
            <IconArrowRight data-arrow width="16" height="16" stroke="#141414"/>
          </div>
        </div>
      </button>
      ...
      <button onClick={()=>goto('gallery')}
        onMouseEnter={e=>{
          ...
          const t = e.currentTarget.querySelector('[data-gallery-title]');
          if (t) { t.style.transform='scale(1.02)'; }
          const a = e.currentTarget.querySelector('[data-gallery-arrow]');
          if (a) a.style.transform='translateX(4px) scale(1.15)';
        }}
        onMouseLeave={e=>{
        ...
        <div style={{display:'flex',alignItems:'flex-end',justifyContent:'space-between',width:'100%',gap:16}}>
          <div style={{minWidth:0}}>
            <div data-gallery-title style={{fontSize:'clamp(44px, 5vw, 72px)',fontWeight:300,letterSpacing:'-0.035em',lineHeight:0.92,color:'#fff',transition:'color .2s ease, transform .3s ease',transformOrigin:'left bottom'}}>
              {lang==='fr'?'Galerie':'Gallery'}
            </div>
          </div>
          <div style={{flexShrink:0}}>
            <IconArrowRight data-gallery-arrow width="16" height="16" stroke="#fff" style={{transition:'transform .25s ease'}}/>
          </div>
        </div>
      </button>
      ...
      <div style={{gridColumn:'1 / 7', gridRow:'2 / 4', background:'#fff', display:'flex', flexDirection:'column', overflow:'hidden', minHeight:0, position:'relative'}}>
        ...
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'24px 18px 0',gap:12,flexShrink:0}}>
          ...
          {/* Toggle — sliding black/orange indicator. Click anywhere to swap. */}
          <button
            ...
            style={{
              ...
              border:'1px solid #141414',padding:3,gap:3,flexShrink:0,
              background:'#fff',cursor:'pointer',fontFamily:'inherit',
            }}>
            {/* Sliding pill */}
            <span aria-hidden="true" style={{
              position:'absolute',top:3,bottom:3,
              left: ecomMode==='type' ? 3 : 'calc(50% + 1.5px)',
              width: 'calc(50% - 4.5px)',
              background: ecomMode==='type' ? '#141414' : 'var(--edo-orange)',
              transition:'left .35s cubic-bezier(.6,.05,.2,1), background .35s ease',
              pointerEvents:'none',
            }}/>
            {[
      ...
      <button onClick={()=>goto('postprod')} style={{...cellBase, gridColumn:'7 / 10', gridRow:'4 / 6', background:'#fff',color:'#141414',padding:'18px 20px', alignSelf:'stretch', height:'calc(100% - 85px)', marginTop:85}} {...hoverGray}>
        ...
        <div style={{display:'flex',alignItems:'flex-end',justifyContent:'space-between',width:'100%',gap:10}}>
          ...
          <IconArrowRight data-arrow width="16" height="16"/>
        </div>
      </button>
